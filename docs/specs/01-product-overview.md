...
| **Workflow** | The engine's holder of an adventure's *global* commands: **Processes** (`preCommands`, run before every parsed sub-command) and **Responses** (`interceptorCommands`, tried as a *fallback* only when no location/pocket command matched the typed verb). The built-in help / inventory / quit / look Responses and the "What now?" (SM2) prompt Process are planted by `CommandFactory.setUpWorkflowCommands`; the author's own are layered on by `WorkflowMapper.populate`. |
