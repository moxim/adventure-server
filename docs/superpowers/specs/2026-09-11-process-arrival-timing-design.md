...
1. **Does anything currently rely on a Process running *before* its sub-command for causal reasons?** No. Queried every Process entry (`workflowData.commands`) across all 3 adventures currently in Mongo for any `SetVariableActionData`/`IncrementVariableActionData`/`DecrementVariableActionData` action — zero matches anywhere. `Workflow.runProcesses()` also never returns anything `GameLoop` checks (`Workflow.java:61-68`, `GameLoop.java:42`), so nothing can use a Process to gate/block the following command either. **A is safe to land with no adventure-content impact.**

2. **Naming and UI placement of the new table.** Follows the existing pattern exactly — `AdventureEditorView` (`AdventureEditorView.java:104-118`) has a "Manage Processes" button → `WorkflowEditorView` (route `author/adventures/:adventureId/workflow`) and a "Manage Responses" button → `ResponsesEditorView` (route `.../responses`), both thin `CommandListEditorView` subclasses differing only in label text, `CommandListType`, and which `WorkflowData` list they bind (`WorkflowEditorView.java:10-23`, `ResponsesEditorView.java:9-22`). Resolved: add a third button **"Manage Arrival"** → `ArrivalProcessesEditorView` (route `author/adventures/:adventureId/arrival`), a `CommandListType.ARRIVAL` entry, and `WorkflowData::getArrivalProcesses` as the fourth constructor arg — same ~15-line shape as the other two, description text: *"An arrival process runs automatically whenever this location's description is shown — when the player arrives by moving here, and every time they explicitly look/describe it. Add preconditions to control whether it actually does anything that time."*

3. **The built-in `"~ ~ ~"` catch-all Process** (`CommandFactory.java:70-73`) prints `SystemMessageKey.SM2` = the literal constant `"What now?"` (`SystemMessageKey.java:30`) — not state-dependent — and is filtered out of visible output entirely by `AdventureRunSession.submit()` (`AdventureRunSession.java:33`). Confirmed inert either way; stays in `commands`, needs no special-casing, unaffected by A's reorder.

4. **Multi-sub-command turns double-firing an arrival Process** (e.g. "go north and look"): confirmed this **will** happen — "go north" fires the arrival hook on entry, "look" fires it again via the describe Response, same turn. Checked against the source material rather than treated as a defect to prevent: PAW's own flow has every `DESC` jump (whatever triggers it — a move, or the built-in look) land back at "Describe Current Location → Search Process Table 1," unconditionally, every time — so Process 1 re-firing on a subsequent explicit "look" is PAW's actual, intended behavior (the Sanec presence-announcement example would equally re-announce "Sanec is here" every time the player re-looks while he's present — the guide's own example carries no "already announced" guard). **No engine-side de-duplication needed or wanted** — an author who wants "only once" adds their own state flag (`NOT_HAS_FLAG`/`SET_FLAG`-equivalent precondition+action pair), exactly as they already must for a Process re-firing across ordinary separate turns under the current (buggy) behavior. Worth one explicit test asserting this double-fire on a conjunction turn, so it's a documented, verified behavior rather than an accident.
