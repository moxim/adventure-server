...
**Goal:** Implement 7 missing condition mappers (plus 1 model change and @Getter additions) to complete the bidirectional mapping between `PreConditionData` subclasses and their runtime `AbstractCondition` counterparts.

**Architecture:** Option A — All 7 mappers extend `PreConditionMapper<DO, BO>`, call `aMapperSupporter.registerMapper(...)` in their constructor, and are annotated `@Service @AutoRegisterMapper`. Leaf conditions use priority 20; composite conditions (And, Or) use priority 10.
...
**`NotConditionMapper` is the canonical composite mapper template.** 
---
