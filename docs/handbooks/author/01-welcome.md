...
| **Workflow** | Global commands, not tied to one location — split into **Processes** (run every turn) and **Responses** (a fallback for a specific typed command that nothing local handles). |
