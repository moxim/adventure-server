...
public class AutoMapperRegistrationProcessor implements BeanPostProcessor {
        ...
        if (annotation != null && bean instanceof Mapper<?, ?> mapper) {
            ...
            if (genericTypes != null && genericTypes.length == 2) {
                ...
                logger.debug("Queued auto-mapper {} for registration: {} <-> {} (priority: {})",
                             beanName, genericTypes[0].getSimpleName(), genericTypes[1].getSimpleName(),
                             annotation.priority());
            } else {
    ...
    private record PendingAutoRegistration(Class<?> dataObjectClass, Class<?> businessObjectClass, Mapper<?, ?> mapper,
                                           int priority, String beanName, String description) {
    }
}
