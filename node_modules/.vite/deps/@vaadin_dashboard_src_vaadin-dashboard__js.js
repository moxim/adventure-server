...
import { a as WRAPPER_LOCAL_NAME, c as getItemsArrayOfItem, d as dashboardLayoutStyles, i as SYNCHRONIZED_ATTRIBUTES, l as itemsEqual, r as getDefaultI18n, s as getElementItem, t as DashboardSection } from "./vaadin-dashboard-section-D8zfjj7_.js";
import "./vaadin-dashboard-widget-B1KTxZ2C.js";
/**
* @license
...
* license.
*/
var dashboardStyles = [dashboardLayoutStyles, css`
  #grid[item-resizing] {
...
`];
//#endregion
//#region node_modules/@vaadin/dashboard/src/vaadin-dashboard-layout-mixin.js
/**
* @license
* Copyright (c) 2000 - 2026 Vaadin Ltd.
*
...
var DashboardLayoutMixin = (superClass) => class DashboardLayoutMixinClass extends ResizeMixin(superClass) {
	...
	*/
	...
	__rootHeadingLevelChanged(rootHeadingLevel) {
		...
		this.dispatchEvent(new CustomEvent("dashboard-root-heading-level-changed", { detail: { value: rootHeadingLevel } }));
	}
};
//#endregion
//#region node_modules/@vaadin/dashboard/src/widget-reorder-controller.js
/**
* @license
* Copyright (c) 2000 - 2026 Vaadin Ltd.
*
...
var WidgetReorderController = class {
	...
	/** @private */
	__dragStart(e) {
		if (![...e.composedPath()].find((el) => el.classList && el.classList.contains("drag-handle"))) return;
		...
		this.draggedElementRemoveObserver.observe(this.host, {
			childList: true,
			subtree: true
		});
	...
	__dragOver(e) {
		...
		if (currentDx && this.__dx && Math.sign(currentDx) !== Math.sign(this.__dx)) this.__startX = e.clientX;
		if (currentDy && this.__dy && Math.sign(currentDy) !== Math.sign(this.__dy)) this.__startY = e.clientY;
		...
		if (!targetElement) targetElement = this.__getClosestElement(otherElements, e.clientX, e.clientY);
		if (!this.__reordering && this.__isDraggedOver(draggedElement, targetElement, {
			...
			const targetIndex = getItemsArrayOfItem(targetItem, this.host.items).indexOf(targetItem);
	...
	__dragEnd() {
		...
		this.__dx = null;
		this.__dy = null;
		if (this.__draggedElement.parentElement === this.host) this.__draggedElement.remove();
	...
	__fireItemMovedEvent(item) {
		...
		this.host.dispatchEvent(new CustomEvent("dashboard-item-moved", { detail: {
			item,
	...
	*/
	__isDraggedOver(draggedElement, targetElement, { x, y, dx, dy }) {
		...
		if (draggedPos.top >= targetPos.bottom) return y < targetPos.bottom && dy < -10;
		else if (draggedPos.bottom <= targetPos.top) return y > targetPos.top && dy > REORDER_MOVE_THRESHOLD;
		else if (draggedPos.left >= targetPos.right) return x < targetPos.right && dx < -10;
		else if (draggedPos.right <= targetPos.left) return x > targetPos.left && dx > REORDER_MOVE_THRESHOLD;
	}
	/**
	...
	*/
	__getDragContextElements() {
		const draggedItemWrapper = [...this.host.querySelectorAll(WRAPPER_LOCAL_NAME)].find((el) => itemsEqual(el.__item, this.draggedItem));
		if (!draggedItemWrapper) return [];
		return [...draggedItemWrapper.parentElement.children].filter((el) => el.localName === "vaadin-dashboard-widget-wrapper" && el.firstElementChild).map((el) => el.firstElementChild);
	}
	/** @private */
	__reorderItems(draggedItem, targetIndex) {
		if (targetIndex < 0 || targetIndex >= getItemsArrayOfItem(draggedItem, this.host.items).length) return;
...
//#endregion
//#region node_modules/@vaadin/dashboard/src/widget-resize-controller.js
/**
* @license
* Copyright (c) 2000 - 2026 Vaadin Ltd.
*
...
var WidgetResizeController = class {
	...
	/** @private */
	__onTrack(e) {
		if (e.detail.state === "start") this.__onResizeStart(e);
		else if (e.detail.state === "track") this.__onResize(e);
		else if (e.detail.state === "end") this.__onResizeEnd(e);
	}
	/** @private */
	__onResizeStart(e) {
		if (![...e.composedPath()].find((el) => el.classList && el.classList.contains("resize-handle"))) return;
		...
		this.__resizedElementRemoveObserver.observe(this.host, {
			childList: true,
			subtree: true
		});
	...
	__onResize(e) {
		...
		if (this.__resizeWidth > currentElementWidth + gapSize + columnWidth / 2) this.__updateResizedItem(1, 0);
		else if (this.__resizeWidth < currentElementWidth - columnWidth / 2) this.__updateResizedItem(-1, 0);
		...
		if (e.detail.ddy > 0 && this.__resizeHeight > currentElementHeight + gapSize + rowMinHeight / 2) this.__updateResizedItem(0, 1);
		else if (e.detail.ddy < 0 && this.__resizeHeight < currentElementHeight - rowMinHeight / 2) this.__updateResizedItem(0, -1);
	}
	/** @private */
	__onResizeEnd() {
		if (!this.resizedItem) return;
		if (this.__resizedElement.parentElement === this.host) this.__resizedElement.remove();
		const itemWrapper = this.__getItemWrapper(this.resizedItem);
		itemWrapper.style.removeProperty("--_widget-resizer-width");
		itemWrapper.style.removeProperty("--_widget-resizer-height");
		if (itemWrapper.firstElementChild) itemWrapper.firstElementChild.toggleAttribute("resizing", false);
	...
	__fireItemResizedEvent(item) {
		this.host.dispatchEvent(new CustomEvent("dashboard-item-resized", { detail: {
			item,
			items: this.host.items
		} }));
	}
	...
	__resizeItem(item, colspanDelta, rowspanDelta) {
		...
		const gridStyle = getComputedStyle(this.host.$.grid);
		if (rowspanDelta && gridStyle.getPropertyValue("--_row-min-height") === "auto") return;
		const columns = gridStyle.gridTemplateColumns.split(" ");
		...
		if ((item.colspan || 1) === newColspan && (item.rowspan || 1) === newRowspan) return;
	...
	__updateWidgetStyles() {
		...
		if (itemWrapper.firstElementChild) itemWrapper.firstElementChild.toggleAttribute("resizing", true);
	}
	/** @private */
...
//#endregion
//#region node_modules/@vaadin/dashboard/src/vaadin-dashboard.js
/**
* @license
* Copyright (c) 2000 - 2026 Vaadin Ltd.
*
...
* license.
*/
var DEFAULT_I18N = getDefaultI18n();
/**
...
* @extends HTMLElement
*/
var Dashboard = class extends DashboardLayoutMixin(I18nMixin(ElementMixin(ThemableMixin(PolylitMixin(LumoInjectionMixin(LitElement)))))) {
	static get is() {
	...
	*/
	...
	__itemsOrRendererChanged(items, renderer) {
		...
		this.querySelectorAll(WRAPPER_LOCAL_NAME).forEach((wrapper) => {
			if (wrapper.firstElementChild && wrapper.firstElementChild.localName === "vaadin-dashboard-section") return;
			...
			} else if (renderer) renderer(wrapper, this, { item });
	...
	__renderItemWrappers(items, hostElement = this) {
		...
		items.forEach((item, index) => {
			...
			if (!wrapper.isConnected) hostElement.appendChild(wrapper);
			...
			if (item.items) {
				...
				if (!isComponentSection) section.sectionTitle = item.title;
		...
		requestAnimationFrame(() => {
			if (focusedWrapperWillBeRemoved) this.__focusWrapperContent(wrapperClosestToRemovedFocused || this.querySelector("vaadin-dashboard-widget-wrapper"));
			else if (focusedElement && !focusedElement.matches(":focus")) focusedElement.focus();
			const focusedItem = this.querySelector(":focus");
			if (focusedItem && this.__outsideViewport(focusedItem)) focusedItem.scrollIntoView();
		});
	}
	/** @private */
	__outsideViewport(element) {
		...
		if (rect.bottom < 0 || rect.right < 0 || rect.top > viewportHeight || rect.left > viewportWidth) return true;
		const dashboardRect = this.getBoundingClientRect();
		if (rect.bottom < dashboardRect.top || rect.right < dashboardRect.left || rect.top > dashboardRect.bottom || rect.left > dashboardRect.right) return true;
		return false;
	}
	/** @private */
	...
	*/
	__isActiveWrapper(wrapper) {
		if (!wrapper || wrapper.localName !== "vaadin-dashboard-widget-wrapper") return false;
	...
	*/
	...
	__getClosestActiveWrapper(wrapper) {
		if (!wrapper || this.__isActiveWrapper(wrapper)) return wrapper;
		...
		const findSiblingWrapper = (wrapper, dir) => {
			while (wrapper) {
				if (this.__isActiveWrapper(wrapper)) return wrapper;
		...
		return findSiblingWrapper(wrapper, 1) || findSiblingWrapper(wrapper, -1) || this.__getClosestActiveWrapper(wrapper.parentElement.closest("vaadin-dashboard-widget-wrapper"));
	}
	/** @private */
	...
	__itemRemove(e) {
		...
		if (beforeRemoveEvent.defaultPrevented) return;
		items.splice(items.indexOf(item), 1);
		this.items = [...this.items];
		this.toggleAttribute("item-selected", false);
		this.dispatchEvent(new CustomEvent("dashboard-item-removed", { detail: {
			item,
			items: this.items,
			section
		} }));
	...
	*/
	__updateColumnCount() {
		...
		if (previousColumnCount !== this.$.grid.style.getPropertyValue("--_col-count")) this.querySelectorAll(WRAPPER_LOCAL_NAME).forEach((wrapper) => {
			if (wrapper.firstElementChild && "requestUpdate" in wrapper.firstElementChild) wrapper.firstElementChild.requestUpdate();
		});
	}
};
