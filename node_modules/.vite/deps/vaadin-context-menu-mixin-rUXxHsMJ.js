...
import "./lit.js";
import { addListener, deepTargetFind, gestures, removeListener } from "./@vaadin_component-base_src_gestures__js.js";
...
import { isAndroid, isIOS, isTouch } from "./@vaadin_component-base_src_browser-utils__js.js";
import { o as OverlayFocusMixin } from "./vaadin-overlay-mixin-Cp6vfu5u.js";
...
*/
var contextMenuItemStyles = [itemStyles, css`
...
`];
//#endregion
//#region node_modules/@vaadin/context-menu/src/styles/vaadin-menu-overlay-base-styles.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
`;
//#endregion
//#region node_modules/@vaadin/context-menu/src/vaadin-menu-overlay-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var MenuOverlayMixin = (superClass) => class MenuOverlayMixin extends OverlayFocusMixin(PositionMixin(superClass)) {
		...
		this.addEventListener("keydown", (e) => {
			if (!e.defaultPrevented && e.composedPath()[0] === this.$.overlay && [38, 40].indexOf(e.keyCode) > -1) {
				...
				if (child && Array.isArray(child.items) && child.items.length) {
					e.preventDefault();
					if (e.keyCode === 38) child.items[child.items.length - 1].focus();
					else child.focus();
				}
			}
		});
	...
	*/
	_updatePosition() {
		...
		if (this.positionTarget && this.parentOverlay && this.opened) {
			...
			if (!!this.style.left) this.style.left = `${parseFloat(this.style.left) + parseFloat(style.paddingLeft)}px`;
			else this.style.right = `${parseFloat(this.style.right) + parseFloat(style.paddingRight)}px`;
			if (!!this.style.bottom) this.style.bottom = `${parseFloat(this.style.bottom) - parseFloat(style.paddingBottom)}px`;
			else this.style.top = `${parseFloat(this.style.top) - parseFloat(style.paddingTop)}px`;
		}
	}
	/**
	...
	*/
	_shouldRestoreFocus() {
		if (this.parentOverlay) return false;
...
//#endregion
//#region node_modules/@vaadin/context-menu/src/vaadin-context-menu-tooltip-controller.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
*/
var ContextMenuTooltipController = class extends SlotController {
	constructor(host) {
		super(host, "tooltip");
	}
	/** @override */
	...
	*/
	open({ trigger }) {
		const tooltipNode = this.node;
		if (tooltipNode?.isConnected && tooltipNode.target) tooltipNode._stateController.open({
			hover: trigger === "hover",
			focus: trigger === "focus"
		});
	}
	bringToFront() {
		this.node?._overlayElement?.bringToFront();
	}
	/**
	* @param {boolean} immediate
	*/
	close(immediate) {
		this.node?._stateController.close(immediate);
	}
};
//#endregion
//#region node_modules/@vaadin/context-menu/src/vaadin-contextmenu-items-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var ItemsMixin = (superClass) => class ItemsMixin extends superClass {
	...
	constructor() {
		...
		this.__itemsOutsideClickListener = (e) => {
			if (this._shouldCloseOnOutsideClick(e)) this.dispatchEvent(new CustomEvent("items-outside-click"));
		};
	...
	*/
	...
	__forwardFocus() {
		...
		if (overlay.parentOverlay) if (overlay.parentOverlay._contentRoot.querySelector("[expanded]")?.hasAttribute("focused") && child) child.focus();
		else overlay.$.overlay.focus();
		else if (child) child.focus();
	}
	/** @private */
	__openSubMenu(subMenu, itemElement) {
		...
		if (parent.hasAttribute("theme")) subMenu.setAttribute("theme", parent.getAttribute("theme"));
		else subMenu.removeAttribute("theme");
		const content = subMenuOverlay.$.content;
		content.style.minWidth = "";
		itemElement.dispatchEvent(new CustomEvent("opensubmenu", { detail: { children: itemElement._item.children } }));
	}
	/** @private */
	...
	*/
	__createComponent(item) {
		let component;
		if (item.component instanceof HTMLElement) component = item.component;
		else component = document.createElement(item.component || `${this._tagNamePrefix}-item`);
		if (component._hasVaadinItemMixin) {
			component.setAttribute("role", "menuitem");
			component.tabIndex = -1;
		}
		if (component.localName === "hr") component.setAttribute("role", "separator");
		else component.setAttribute("aria-haspopup", "false");
		this._setMenuItemTheme(component, item, this._theme);
		component._item = item;
		if (item.text) component.textContent = item.text;
		if (item.className) component.setAttribute("class", item.className);
	...
	__initListBox() {
		...
		if (this._theme) listBox.setAttribute("theme", this._theme);
		...
		listBox.addEventListener("selected-changed", (event) => {
			...
			if (typeof value === "number") {
				...
				if (!item.children) this.dispatchEvent(new CustomEvent("item-selected", { detail: { value: item } }));
			}
		});
		return listBox;
	}
	...
	__initOverlay() {
		...
		overlay.addEventListener("keydown", (event) => {
			...
			if (!isRTL && isArrowRight || isRTL && isArrowLeft || key === "Enter" || key === " ") this.__showSubMenu(event);
			else if (!isRTL && isArrowLeft || isRTL && isArrowRight || key === "Escape") {
				if (key === "Escape") event.stopPropagation();
				this.close();
				this.listenOn.focus();
			} else if (key === "Tab" && !event.defaultPrevented) this.dispatchEvent(new CustomEvent("close-all-menus"));
		});
	}
	/** @private */
	__initSubMenu() {
		...
		this.addEventListener("opened-changed", (event) => {
			if (!event.detail.value) this._subMenu.close();
		});
		...
		this.addEventListener("item-selected", (e) => {
			...
			} else if (!selectedItem.keepOpen) this.close();
		});
		subMenu.addEventListener("opened-changed", (event) => {
			if (event.detail.value) this._tooltipController.bringToFront();
			else {
				const expandedItem = this._listBox.querySelector("[expanded]");
				if (expandedItem) this.__updateExpanded(expandedItem, false);
			}
		});
		return subMenu;
	}
	...
	__showSubMenu(event, item = event.composedPath().find((node) => node.localName === `${this._tagNamePrefix}-item`)) {
		if (!this.__openListenerActive) return;
		...
		if (item && item !== expandedItem && !item.disabled) {
			...
			const isSubmenuFocused = subMenu._overlayElement._contentRoot.firstElementChild?.focused;
			if (expandedItem) this.__updateExpanded(expandedItem, false);
			if (!children || !children.length) subMenu.close();
			if (!this.opened) return;
			if (children?.length) {
				this.__updateExpanded(item, true);
				this.__openSubMenu(subMenu, item);
			} else if (isSubmenuFocused) subMenu.listenOn.focus();
			else if (!this._listBox.focused) this._overlayElement.$.overlay.focus();
		}
		if (item?.disabled) subMenu.close();
	}
	/** @protected */
	__getListBox() {
	...
	*/
	...
	_setMenuItemTheme(component, item, hostTheme) {
		...
		if (item.theme != null) theme = Array.isArray(item.theme) ? item.theme.join(" ") : item.theme;
	...
	__initMenu(root, _menu) {
		...
		} else this.__updateTheme(this._listBox, this._theme);
	}
	/** @private */
	...
	__updateTheme(component, theme) {
		if (theme) component.setAttribute("theme", theme);
		else component.removeAttribute("theme");
	}
	close() {
		if (!this.__parentMenu) this._tooltipController.close(true);
	}
};
//#endregion
//#region node_modules/@vaadin/context-menu/src/vaadin-context-menu-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var ContextMenuMixin = (superClass) => class ContextMenuMixinClass extends ItemsMixin(superClass) {
	...
	connectedCallback() {
		...
		if (this.__restoreOpened) this._setOpened(true);
	}
	/** @protected */
	disconnectedCallback() {
		super.disconnectedCallback();
	...
	*/
	_onOverlayOpened(event) {
		if (event.target !== this._overlayElement) return;
		const opened = event.detail.value;
		this._setOpened(opened);
		if (opened) this.__alignOverlayPosition();
	}
	/**
	...
	*/
	...
	_rendererChanged(renderer, items) {
		if (items) {
			if (renderer) throw new Error("The items API cannot be used together with a renderer");
			if (this.closeOn === "click") this.closeOn = "";
		}
	}
	/**
	* Closes the overlay.
	*/
	...
	_contextTarget(e) {
		...
		} else if (this.listenOn && this.listenOn !== this && this.position) return this.listenOn;
		return e.target;
	}
	/**
	* Opens the overlay.
	...
	*/
	open(e) {
		if (this._overlayElement && e.composedPath().includes(this._overlayElement)) return;
		if (e && !this.opened) {
			this._context = {
				detail: e.detail,
	...
	__preserveMenuState() {
		...
		if (listBox) {
			...
			if (this._subMenu && this._subMenu.opened) this.__subMenuIndex = listBox.items.indexOf(this._subMenu.listenOn);
		}
	}
	/** @private */
	__restoreMenuState() {
		...
		if (listBox) {
			...
			if (subMenuIndex > -1) {
				...
				if (itemToOpen) if (Array.isArray(itemToOpen._item.children) && itemToOpen._item.children.length) {
	...
	/** @private */
	__focusItem(item) {
		if (item) item.focus({ focusVisible: isKeyboardActive() });
	}
	/** @private */
	__onScroll() {
		if (!this.opened || this.position) return;
	...
	__alignOverlayPosition() {
		const overlay = this._overlayElement;
		if (overlay.positionTarget) return;
		...
		[
			...
			"end-aligned",
			"bottom-aligned"
		].forEach((attr) => overlay.removeAttribute(attr));
		...
		if (!this.__isRTL) if (x < wdthVport / 2 || x < xMax) style.left = `${x}px`;
		else {
			style.right = `${Math.max(0, wdthVport - x)}px`;
			this._setEndAligned(overlay);
		}
		else if (x > wdthVport / 2 || x > xMin) style.right = `${Math.max(0, wdthVport - x)}px`;
		else {
			style.left = `${x}px`;
			this._setEndAligned(overlay);
		}
		if (y < hghtVport / 2 || y < yMax) style.top = `${y}px`;
		else {
	...
	_setEndAligned(element) {
		...
		if (!this.__isRTL) element.setAttribute("right-aligned", "");
	}
	/** @private */
	_getEventCoordinate(event, coord) {
		if (event.detail instanceof Object) {
			if (event.detail[coord]) return event.detail[coord];
			else if (event.detail.sourceEvent) return this._getEventCoordinate(event.detail.sourceEvent, coord);
		} else {
	...
	_listen(node, evType, handler) {
		if (gestures[evType]) addListener(node, evType, handler);
		else node.addEventListener(evType, handler);
	}
	/** @private */
	_unlisten(node, evType, handler) {
		if (gestures[evType]) removeListener(node, evType, handler);
		else node.removeEventListener(evType, handler);
	}
	/** @private */
	...
	__onGlobalContextMenu(e) {
		...
		if (!e.shiftKey) {
			if (!(isAndroid || isIOS)) {
				...
				this._overlayElement.addEventListener("vaadin-overlay-closed", (closeEvent) => {
					if (closeEvent.target === this._overlayElement) this.__contextMenuAt(e.clientX, e.clientY);
				}, { once: true });
			}
			e.preventDefault();
			this.close();
...
//#endregion
export { contextMenuItemStyles as a, menuOverlayStyles as i, ContextMenuTooltipController as n, MenuOverlayMixin as r, ContextMenuMixin as t };

//# sourceMappingURL=vaadin-context-menu-mixin-rUXxHsMJ.js.map