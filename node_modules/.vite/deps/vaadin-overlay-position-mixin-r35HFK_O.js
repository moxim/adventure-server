import { r as getAncestorRootNodes } from "./dom-utils-DDAqRijW.js";
import { n as observeMove, r as setOverlayStateAttribute } from "./vaadin-overlay-mixin-Cp6vfu5u.js";
//#region node_modules/@vaadin/overlay/src/vaadin-overlay-position-mixin.js
/**
* @license
...
var targetResizeObserver = new ResizeObserver((entries) => {
	setTimeout(() => {
		entries.forEach((entry) => {
			if (entry.target.__overlay) entry.target.__overlay._updatePosition();
		});
	});
});
var PositionMixin = (superClass) => class PositionMixin extends superClass {
	...
	connectedCallback() {
		...
		if (this.opened) this.__addUpdatePositionEventListeners();
	}
	/** @protected */
	...
	updated(props) {
		...
		if (props.has("positionTarget")) {
			...
			if (!this.positionTarget && oldTarget || this.positionTarget && !oldTarget && !!this.__margins) this.__resetPosition();
		}
		if (props.has("opened") || props.has("positionTarget")) this.__updatePositionSettings(this.opened, this.positionTarget);
		if ([
			"horizontalAlign",
			"verticalAlign",
			"noHorizontalOverlap",
			"noVerticalOverlap",
			"requiredVerticalSpace"
		].some((prop) => props.has(prop))) this._updatePosition();
	}
	/** @private */
	__addUpdatePositionEventListeners() {
		...
		if (this.positionTarget) this.__observePositionTargetMove = observeMove(this.positionTarget, () => {
	...
	/** @private */
	__onScroll(e) {
		if (e.target instanceof Node && this._deepContains(e.target)) return;
		this._updatePosition();
	}
	/** @private */
	__resetPosition() {
	...
	_updatePosition() {
		if (!this.positionTarget || !this.opened || !this.__margins) return;
	...
	__shouldAlignStart(targetRect, contentSize, viewportSize, margins, defaultAlignStart, noOverlap, propNames) {
		...
		return defaultAlignStart === (spaceForDefaultAlignment > (defaultAlignStart ? spaceForEndAlignment : spaceForStartAlignment) || spaceForDefaultAlignment > contentSize);
	}
	/**
	...
	__adjustBottomProperty(cssPropNameToSet, propNames, currentValue) {
		...
		if (cssPropNameToSet === propNames.end) {
			if (propNames.end === PROP_NAMES_VERTICAL.end) {
				const viewportHeight = Math.min(window.innerHeight, document.documentElement.clientHeight);
				if (currentValue > viewportHeight && this.__oldViewportHeight) adjustedProp = currentValue - (this.__oldViewportHeight - viewportHeight);
				this.__oldViewportHeight = viewportHeight;
			}
			if (propNames.end === PROP_NAMES_HORIZONTAL.end) {
				const viewportWidth = Math.min(window.innerWidth, document.documentElement.clientWidth);
				if (currentValue > viewportWidth && this.__oldViewportWidth) adjustedProp = currentValue - (this.__oldViewportWidth - viewportWidth);
...
//#endregion
export { PositionMixin as t };

//# sourceMappingURL=vaadin-overlay-position-mixin-r35HFK_O.js.map