//#region node_modules/@vaadin/upload/src/vaadin-upload-helpers.js
/**
* @license
...
function getFilesFromDropEvent(dropEvent) {
	async function getFilesFromEntry(entry) {
		if (entry.isFile) return new Promise((resolve) => {
			...
			return (await Promise.all(entries.map(getFilesFromEntry))).flat();
		}
	}
	if (!Array.from(dropEvent.dataTransfer.items).filter((item) => !!item).filter((item) => typeof item.webkitGetAsEntry === "function").map((item) => item.webkitGetAsEntry()).some((entry) => !!entry?.isDirectory)) return Promise.resolve(dropEvent.dataTransfer.files ? Array.from(dropEvent.dataTransfer.files) : []);
...
//#endregion
export { getFilesFromDropEvent as t };

//# sourceMappingURL=vaadin-upload-helpers-gUhRt1Vt.js.map