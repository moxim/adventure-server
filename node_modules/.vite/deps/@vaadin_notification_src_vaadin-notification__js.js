...
import { r as isTemplateResult } from "./directive-helpers-DFVz-hGr.js";
import "./directive-helpers-CTkTaP8K.js";
//#region node_modules/@vaadin/notification/src/styles/vaadin-notification-card-base-styles.js
/**
* @license
...
`;
//#endregion
//#region node_modules/@vaadin/notification/src/styles/vaadin-notification-container-base-styles.js
/**
* @license
* Copyright (c) 2017 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
`;
//#endregion
//#region node_modules/@vaadin/component-base/src/overlay-class-mixin.js
/**
* @license
...
var OverlayClassMixin = (superclass) => class OverlayClassMixinClass extends superclass {
	...
	__updateOverlayClassNames(overlayClass, overlayElement) {
		...
		if (overlayClass === void 0) return;
		const { classList } = overlayElement;
		if (!this.__initialClasses) this.__initialClasses = new Set(classList);
		if (Array.isArray(this.__previousClasses)) {
			...
			if (classesToRemove.length > 0) classList.remove(...classesToRemove);
		}
		const classesToAdd = typeof overlayClass === "string" ? overlayClass.split(" ").filter(Boolean) : [];
		if (classesToAdd.length > 0) classList.add(...classesToAdd);
		this.__previousClasses = classesToAdd;
	}
};
//#endregion
//#region node_modules/@vaadin/notification/src/vaadin-notification-mixin.js
/**
* @license
* Copyright (c) 2017 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
opened: {
	...
	*/
	...
	_detectIosNavbar() {
		...
		if (landscape && clientHeight > innerHeight) this.style.bottom = `${clientHeight - innerHeight}px`;
		else this.style.bottom = "0";
	}
	/** @private */
	_onVaadinOverlayClose(event) {
		if (event.detail.sourceEvent?.composedPath().indexOf(this) >= 0) event.preventDefault();
	}
};
/**
...
var NotificationMixin = (superClass) => class extends ThemePropertyMixin(OverlayClassMixin(superClass)) {
	...
	*/
	static show(contents, options) {
		...
		if (isTemplateResult(contents)) return Notification._createAndShowNotification((root) => {
	...
	static _createAndShowNotification(renderer, options) {
		...
		if (options && Number.isFinite(options.duration)) notification.duration = options.duration;
		if (options?.position) notification.position = options.position;
		if (options?.assertive) notification.assertive = options.assertive;
		if (options?.theme) notification.setAttribute("theme", options.theme);
		if (options?.className) notification.overlayClass = options.className;
		notification.renderer = renderer;
		document.body.appendChild(notification);
		notification.opened = true;
		notification.addEventListener("opened-changed", (e) => {
			if (!e.detail.value) notification.remove();
		});
		return notification;
	}
	/** @private */
	...
	disconnectedCallback() {
		...
		queueMicrotask(() => {
			if (!this.isConnected) this.opened = false;
		});
	}
	/**
	...
	*/
	requestContentUpdate() {
		if (!this.renderer || !this._card) return;
	...
	_rendererChanged(renderer, opened, card) {
		...
		if (opened) {
			if (!this._didAnimateNotificationAppend) this._animatedAppendNotificationCard();
	...
	*/
	...
	_openedChanged(opened) {
		...
		} else if (this._card) this._closeNotificationCard();
	}
	/** @private */
	...
	_animatedAppendNotificationCard() {
		...
		} else this._didAnimateNotificationAppend = false;
	}
	/** @private */
	_appendNotificationCard() {
		...
		if (this._container.firstElementChild) this._container.bringToFront();
		this._card.slot = this.position;
		if (this._container.firstElementChild && /top/u.test(this.position)) this._container.insertBefore(this._card, this._container.firstElementChild);
		else this._container.appendChild(this._card);
	}
	/** @private */
	...
	_closeNotificationCard() {
		if (this._durationTimeoutId) clearTimeout(this._durationTimeoutId);
	...
	_animatedRemoveNotificationCard() {
		...
		if (name && name !== "none") {
			...
			this._card.addEventListener("animationend", this.__animationEndListener);
		} else this._removeNotificationCard();
	}
	/** @private */
	_positionChanged() {
		if (this.opened) this._animatedAppendNotificationCard();
	}
	/** @private */
	_durationChanged(duration, opened) {
		if (opened) {
			clearTimeout(this._durationTimeoutId);
			if (duration > 0) this._durationTimeoutId = setTimeout(() => this.close(), duration);
		}
	}
};
//#endregion
//#region node_modules/@vaadin/notification/src/vaadin-notification.js
/**
* @license
* Copyright (c) 2017 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
* @private
*/
var NotificationContainer = class extends NotificationContainerMixin(ThemableMixin(ElementMixin(PolylitMixin(LumoInjectionMixin(LitElement))))) {
	static get is() {
