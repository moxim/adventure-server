...
import { i as isElementFocused } from "./focus-utils-CzjQXswF.js";
...
import { n as sliderStyles, r as styleMap, t as SliderMixin } from "./vaadin-slider-mixin-DtcirS2Z.js";
//#region node_modules/@vaadin/slider/src/vaadin-range-slider.js
/**
* @license
...
* @extends HTMLElement
*/
var RangeSlider = class extends FieldMixin(SliderMixin(FocusMixin(ElementMixin(ThemableMixin(PolylitMixin(LumoInjectionMixin(LitElement))))))) {
	static get is() {
	...
	static get properties() {
		return {
			...
			*/
			accessibleNameStart: { type: String },
			/**
	...
	render() {
      ...
      <div class="vaadin-slider-container">
        ...
        <div id="controls" style="${styleMap({
	...
	*/
	focus(options) {
		...
		if (this._inputElements) this._inputElements[0].focus();
		super.focus(options);
	}
	/**
	* @protected
	...
	*/
	...
	__onStartInput(event) {
		...
		if (parseFloat(event.target.value) > this.__value[1]) event.target.value = this.__value[1];
	...
	__onEndInput(event) {
		event.stopPropagation();
		if (parseFloat(event.target.value) < this.__value[0]) event.target.value = this.__value[0];
		const value = event.target.value;
	...
	__onStartPointerEnter(event) {
		if (this.__isThumbEvent(event, this.__thumbStartElement)) this.__startHover = true;
	}
	/** @private */
	...
	__onEndPointerEnter(event) {
		if (this.__isThumbEvent(event, this.__thumbEndElement)) this.__endHover = true;
	}
	/** @private */
	...
	__onKeyDown(event) {
		...
		if (this.readonly || this.__value[0] === this.__value[1] && (index === 0 && isNextKey || index === 1 && isPrevKey)) event.preventDefault();
	}
	/** @private */
	__updateBubble(idx) {
...
//# sourceMappingURL=@vaadin_slider_src_vaadin-range-slider__js.js.map