...
import { s as removeValueFromAttribute, t as addValueToAttribute } from "./dom-utils-DDAqRijW.js";
...
import { c as overlayStyles } from "./vaadin-overlay-mixin-Cp6vfu5u.js";
import { n as PopoverPositionMixin, r as PopoverOverlayMixin, t as PopoverTargetMixin } from "./vaadin-popover-target-mixin-nZAu-I64.js";
import { t as ifDefined } from "./if-defined-HGTsiUgW.js";
//#region node_modules/@vaadin/tooltip/src/styles/vaadin-tooltip-overlay-base-styles.js
/**
* @license
...
`;
//#endregion
//#region node_modules/@vaadin/tooltip/src/vaadin-tooltip-overlay.js
/**
* @license
* Copyright (c) 2022 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
* @private
*/
var TooltipOverlay = class extends PopoverOverlayMixin(DirMixin(ThemableMixin(PolylitMixin(LumoInjectionMixin(LitElement))))) {
	static get is() {
...
//#endregion
//#region node_modules/@vaadin/tooltip/src/vaadin-tooltip-mixin.js
/**
* @license
* Copyright (c) 2022 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var TooltipStateController = class {
	...
	*/
	open(options = { immediate: false }) {
		...
		if (!immediate && (isHover || isFocus) && !this.__closeTimeout) this.__warmupTooltip(isFocus);
		else this.__showTooltip();
	}
	/**
	...
	*/
	close(immediate) {
		if (!immediate && this.hideDelay > 0) this.__scheduleClose();
		else {
			this.__abortClose();
	...
	__warmupTooltip(isFocus) {
		if (!this._isOpened()) {
			if (warmedUp) this.__showTooltip();
			else if (warmUpTimeout == null) this.__scheduleWarmUp(isFocus);
		}
	}
	/** @private */
	__abortClose() {
		...
		if (this.isClosing) closing.delete(this.host);
	}
	/** @private */
	__abortCooldown() {
		if (cooldownTimeout) {
...
*/
var TooltipMixin = (superClass) => class TooltipMixinClass extends PopoverPositionMixin(PopoverTargetMixin(superClass)) {
	...
	*/
	...
	disconnectedCallback() {
		...
		if (this.opened && !this.manual) this._stateController.close(true);
	...
	ready() {
		...
		this.__contentController = new SlotController(this, "overlay", "div", { initializer: (element) => {
	...
	updated(props) {
		super.updated(props);
		if (props.has("text") || props.has("generator") || props.has("context") || props.has("markdown")) this.__updateContent();
	...
	__openedChanged(opened, oldOpened) {
		if (opened) document.addEventListener("keydown", this.__onKeyDown, true);
		else if (oldOpened) document.removeEventListener("keydown", this.__onKeyDown, true);
	}
	/**
	...
	*/
	...
	__onFocusin(event) {
		...
		if (!this.__isShouldShow()) return;
		if (this._overlayElement.hasAttribute("hidden")) return;
		this.__focusInside = true;
		if (!this.__isTargetHidden && (!this.__hoverInside || !this.opened)) this._stateController.open({ focus: true });
	}
	/** @private */
	__onFocusout(event) {
		if (this.manual) return;
		...
		if (!this.__hoverInside) this._stateController.close(true);
	}
	/** @private */
	__onKeyDown(event) {
		if (this.manual) return;
	...
	__onMouseEnter() {
		...
		if (!this.__isShouldShow()) return;
		if (this._overlayElement.hasAttribute("hidden")) return;
		if (this.__hoverInside) return;
		this.__hoverInside = true;
		if (!this.__isTargetHidden && (!this.__focusInside || !this.opened)) this._stateController.open({ hover: true });
	}
	/** @private */
	__onMouseLeave(event) {
		if (event.relatedTarget !== this._overlayElement) this.__handleMouseLeave();
	}
	/** @protected */
	__onOverlayMouseEnter() {
		if (this.manual) return;
		if (this._stateController.isClosing) this._stateController.open({ immediate: true });
	}
	/** @protected */
	__onOverlayMouseLeave(event) {
		if (event.relatedTarget !== this.target) this.__handleMouseLeave();
	}
	/** @protected */
	...
	__handleMouseLeave() {
		...
		if (!this.__focusInside) this._stateController.close();
	}
	/** @private */
	__onOverlayOpen() {
		if (this.manual) return;
		if (this._overlayElement.opened && !this._overlayElement._last) this._stateController.close(true);
	}
	/** @private */
	__onTargetVisibilityChange(isVisible) {
		...
		if (!isVisible && this.opened) this._stateController.close(true);
	}
	/** @private */
	__isShouldShow() {
		if (typeof this.shouldShow === "function" && this.shouldShow(this.target, this.context) !== true) return false;
		return true;
	}
	/** @private */
	async __updateContent() {
		...
		if (this.markdown && content) (await this.constructor.__importMarkdownHelpers()).renderMarkdownToElement(this.__contentNode, content);
		else this.__contentNode.textContent = content || "";
		...
		if (newTextContent !== oldTextContent) this.dispatchEvent(new CustomEvent("content-changed", { detail: { content: newTextContent } }));
	}
	/** @private */
	...
	__updateAriaAttributes(oldTarget, oldMode) {
		if (oldTarget && oldMode && oldMode !== "none") [oldTarget].flat().forEach((target) => {
			removeValueFromAttribute(target, oldMode, this._uniqueId);
		});
		const target = this._effectiveAriaTarget;
		const mode = this.ariaLinkMode;
		if (target && mode && mode !== "none") [target].flat().forEach((el) => {
			addValueToAttribute(el, mode, this._uniqueId);
		});
	}
	/** @private **/
	static __importMarkdownHelpers() {
		if (!this.__markdownHelpers) this.__markdownHelpers = import("./markdown-helpers-ZWp6J23U.js").then((n) => n.t);
		return this.__markdownHelpers;
	}
};
//#endregion
//#region node_modules/@vaadin/tooltip/src/vaadin-tooltip.js
/**
* @license
* Copyright (c) 2022 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
* @extends HTMLElement
*/
var Tooltip = class extends TooltipMixin(ThemePropertyMixin(ElementMixin(PolylitMixin(LitElement)))) {
	static get is() {
...
//# sourceMappingURL=vaadin-tooltip-oaZl5vRv.js.map