import { i as CLASS_UNSELECTABLE } from "./css-DK5RcH5d.js";
import { t as Control } from "./Control-Bot7nb2G.js";
import { R as METERS_PER_UNIT, b as getPointResolution } from "./proj-R0q3-kiO.js";
...
* @api
*/
var ScaleLine = class extends Control {
	/**
	...
	*/
	updateElement_() {
		...
		if (units == "degrees") {
		...
		} else if (units == "imperial") if (nominalCount < .9144) {
			suffix = "in";
			...
			suffix = "NM";
		} else if (units == "metric") if (nominalCount < 1e-6) {
			suffix = "nm";
			pointResolution *= 1e9;
		...
		else if (units == "us") if (nominalCount < .9144) {
			suffix = "in";
	...
	*/
	createScaleBar(width, scale, suffix) {
		...
		for (let i = 0; i < steps; ++i) {
			...
			scaleSteps.push(`<div><div class="ol-scale-singlebar ${cls}" style="width: ${stepWidth}px;"></div>` + this.createMarker("relative") + (i % 2 === 0 || steps === 2 ? this.createStepText(i, width, false, scale, suffix) : "") + "</div>");
		}
		scaleSteps.push(this.createStepText(steps, width, true, scale, suffix));
		return (this.scaleBarText_ ? `<div class="ol-scale-text" style="width: ${width}px;">` + mapScale + "</div>" : "") + scaleSteps.join("");
	}
	/**
	...
	*/
	createMarker(position) {
		return `<div class="ol-scale-step-marker" style="position: ${position}; top: ${position === "absolute" ? 3 : -10}px;"></div>`;
	}
	/**
	...
	*/
	createStepText(i, width, isLast, scale, suffix) {
		const lengthString = (i === 0 ? 0 : Math.round(scale / this.scaleBarSteps_ * i * 100) / 100) + (i === 0 ? "" : " " + suffix);
	...
	*/
	getScaleForResolution() {
		...
		return resolution * (1e3 / 25.4) * dpi;
	}
	/**
