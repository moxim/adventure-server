...
//#region node_modules/@vaadin/component-base/src/slot-child-observe-controller.js
/**
* @license
...
var SlotChildObserveController = class extends SlotController {
	...
	*/
	teardownNode(_node) {
		...
		if (node && node !== this.defaultNode) this.__notifyChange(node);
		else {
	...
	*/
	attachDefaultNode() {
		...
		if (node) this.__updateNodeId(node);
		return node;
	}
	/**
	...
	*/
	observeNode(node) {
		if (this.__nodeObserver) this.__nodeObserver.disconnect();
		this.__nodeObserver = new MutationObserver((mutations) => {
			mutations.forEach((mutation) => {
				const target = mutation.target;
				const isCurrentNodeMutation = target === this.node;
				if (mutation.type === "attributes") {
					if (isCurrentNodeMutation) this.__updateNodeId(target);
				} else if (isCurrentNodeMutation || target.parentElement === this.node) this.__notifyChange(this.node);
			});
		});
	...
	*/
	__notifyChange(node) {
		this.dispatchEvent(new CustomEvent("slot-content-changed", { detail: {
	...
	*/
	__updateNodeId(node) {
		...
		if (node.nodeType === Node.ELEMENT_NODE && (!this.multiple || isFirstNode) && !node.id) node.id = this.defaultId;
	}
};
//#endregion
export { SlotChildObserveController as t };

//# sourceMappingURL=slot-child-observe-controller-DRwVKtF7.js.map