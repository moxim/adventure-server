//#region node_modules/@vaadin/component-base/src/data-provider-controller/cache.js
/**
* @license
...
var Cache = class Cache {
	...
	*/
	get isLoading() {
		if (Object.keys(this.pendingRequests).length > 0) return true;
	...
	*/
	set size(size) {
		...
		if (this.context.placeholder !== void 0) {
			...
			for (let i = 0; i < size || 0; i++) this.items[i] ||= this.context.placeholder;
		}
		if (this.items.length > size) this.items.length = size || 0;
		Object.keys(this.pendingRequests).forEach((page) => {
			if (parseInt(page) * this.pageSize >= this.size || 0) delete this.pendingRequests[page];
		});
	}
	/**
	...
	*/
	setPage(page, items) {
		...
		items.forEach((item, i) => {
			...
			if (this.size === void 0 || itemIndex < this.size) this.items[itemIndex] = item;
		});
	}
	/**
	...
	*/
	getFlatIndex(index) {
		...
		return this.subCaches.reduce((prev, subCache) => {
			return clampedIndex > subCache.parentCacheIndex ? prev + subCache.flatSize : prev;
		}, clampedIndex);
	}
};
//#endregion
//#region node_modules/@vaadin/component-base/src/data-provider-controller/helpers.js
/**
* @license
* Copyright (c) 2021 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
function getFlatIndexContext(cache, flatIndex, level = 0) {
	...
	for (const subCache of cache.subCaches) {
		...
		if (levelIndex <= index) break;
		else if (levelIndex <= index + subCache.flatSize) return getFlatIndexContext(subCache, levelIndex - index - 1, level + 1);
...
function getItemContext({ getItemId }, cache, targetItem, level = 0, levelFlatIndex = 0) {
	for (let index = 0; index < cache.items.length; index++) {
		...
		if (!!item && getItemId(item) === getItemId(targetItem)) return {
			cache,
			level,
			item,
			index,
...
function getFlatIndexByPath(cache, [levelIndex, ...subIndexes], flatIndex = 0) {
	if (levelIndex === Infinity) levelIndex = cache.size - 1;
	const flatIndexOnLevel = cache.getFlatIndex(levelIndex);
	const subCache = cache.getSubCache(levelIndex);
	if (subCache?.flatSize > 0 && subIndexes.length) return getFlatIndexByPath(subCache, subIndexes, flatIndex + flatIndexOnLevel + 1);
	return flatIndex + flatIndexOnLevel;
}
//#endregion
//#region node_modules/@vaadin/component-base/src/data-provider-controller/data-provider-controller.js
/**
* @license
* Copyright (c) 2021 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var DataProviderController = class extends EventTarget {
	...
	*/
	ensureFlatIndexLoaded(flatIndex) {
		...
		if (!this.#isItemLoaded(item)) this.#loadCachePage(cache, page);
	}
	/**
	...
	*/
	...
	#loadCachePage(cache, page) {
		if (!this.dataProvider || cache.pendingRequests[page] || !this._shouldLoadCachePage(cache, page)) return;
		let params = {
			page,
			pageSize: this.pageSize,
			parentItem: cache.parentItem
		};
		if (this.dataProviderParams) params = {
			...params,
			...this.dataProviderParams()
		};
		const callback = (items, size) => {
			if (cache.pendingRequests[page] !== callback) return;
			if (size !== void 0) cache.size = size;
	...
	/** @private */
	#isItemLoaded(item) {
		if (this.isPlaceholder) return !this.isPlaceholder(item);
		else if (this.placeholder) return item !== this.placeholder;
		return !!item;
	}
};
//#endregion
export { DataProviderController as t };

//# sourceMappingURL=data-provider-controller-Daps3WDq.js.map