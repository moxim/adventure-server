...
import { animationFrame, microTask, timeOut } from "./@vaadin_component-base_src_async__js.js";
import { addListener, setTouchAction } from "./@vaadin_component-base_src_gestures__js.js";
import { n as get$1, t as PolylitMixin } from "./polylit-mixin-BwU08z1T.js";
...
import { a as isElementHidden, o as isKeyboardActive } from "./focus-utils-CzjQXswF.js";
import { t as TabindexMixin } from "./tabindex-mixin-T83laqFC.js";
...
import { isAndroid, isChrome, isFirefox, isIOS, isSafari, isTouch } from "./@vaadin_component-base_src_browser-utils__js.js";
import { t as ifDefined } from "./if-defined-HGTsiUgW.js";
import { t as getNormalizedScrollLeft } from "./dir-utils-9YTU0Dgy.js";
import { t as Virtualizer } from "./virtualizer-DLVbY-3F.js";
import { t as DataProviderController } from "./data-provider-controller-Daps3WDq.js";
import { ColumnObserver, findTreeToggleCell, getBodyRowCells, iterateChildren, iterateRowCells, updateBooleanRowStates, updateCellState, updateCellsPart, updateColumnOrders, updatePart, updateState, updateStringRowStates } from "./@vaadin_grid_src_vaadin-grid-helpers__js.js";
import "./@vaadin_grid_src_vaadin-grid-column__js.js";
import { ActiveItemMixin } from "./@vaadin_grid_src_vaadin-grid-active-item-mixin__js.js";
import { t as OverflowController } from "./overflow-controller-B1O3h4Je.js";
//#region node_modules/@vaadin/grid/src/styles/vaadin-grid-base-styles.js
/**
* @license
...
`;
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-a11y-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
accessibleName: { type: String } };
	...
	__a11yUpdateGridSize(size, _columnTree, emptyState) {
		if (size === void 0 || _columnTree === void 0) return;
		...
		const rowsCount = (emptyState ? 1 : size) + headerRowsCount + footerRowsCount;
	...
	*/
	__a11yUpdateRowExpanded(row) {
		...
		if (this.__isRowExpandable(row)) {
			...
			if (toggleCell) toggleCell.setAttribute("aria-expanded", "false");
		} else if (this.__isRowCollapsible(row)) {
			row.setAttribute("aria-expanded", "true");
			if (toggleCell) toggleCell.setAttribute("aria-expanded", "true");
		} else {
			row.removeAttribute("aria-expanded");
			if (toggleCell) toggleCell.removeAttribute("aria-expanded");
		}
	}
	/**
	* @param {!HTMLElement} row
	...
	*/
	...
	__a11yUpdateSorters() {
		Array.from(this.querySelectorAll("vaadin-grid-sorter")).forEach((sorter) => {
			...
			if (cellContent?.assignedSlot) cellContent.assignedSlot.parentNode.setAttribute("aria-sort", {
				asc: "ascending",
				desc: "descending"
...
//#endregion
//#region node_modules/@vaadin/grid/src/array-data-provider.js
/**
* @license
...
function checkPaths(arrayToCheck, action, items) {
	...
	arrayToCheck.forEach(({ path }) => {
		...
		if (get(path.replace(/\.[^.]*$/u, ""), items[0]) === void 0) {
...
function normalizeEmptyValue(value) {
	if ([void 0, null].indexOf(value) >= 0) return "";
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-array-data-provider-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
items: {
	...
	*/
	_onDataProviderPageReceived() {
		...
		if (this._arrayDataProvider) this.size = this._flatSize;
	}
	/** @private */
	__dataProviderOrItemsChanged(dataProvider, items, isAttached) {
		if (!isAttached) return;
		if (this._arrayDataProvider) if (dataProvider !== this._arrayDataProvider) {
			...
			this.size = 0;
			this.clearCache();
		} else if (this._arrayDataProvider.__items === items) this.clearCache();
		else this.__setArrayDataProvider(items);
		else if (items) this.__setArrayDataProvider(items);
	}
};
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-column-auto-width-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
__pendingRecalculateColumnWidths: {
	...
	updated(props) {
		super.updated(props);
		if (props.has("__hostVisible") && !props.get("__hostVisible")) this.__tryToRecalculateColumnWidthsIfPending();
	}
	/** @private */
	__dataProviderChangedAutoWidth(_dataProvider) {
		if (this.__hasHadRenderedRowsForColumnWidthCalculation) return;
	...
	__flatSizeChangedAutoWidth(flatSize) {
		requestAnimationFrame(() => {
			if (!!flatSize && !this.__hasHadRenderedRowsForColumnWidthCalculation) this.recalculateColumnWidths();
			else this.__tryToRecalculateColumnWidthsIfPending();
		});
	}
	/**
	* @protected
	...
	*/
	...
	__getIntrinsicWidth(col) {
		if (!this.__intrinsicWidthCache.has(col)) this.__calculateAndCacheIntrinsicWidths([col]);
	...
	__getDistributedWidth(col, innerColumn) {
		...
		const shareOfInnerColumnFromNecessaryExtraSpace = this.__getIntrinsicWidth(innerColumn) / sumOfWidthOfAllChildColumns * extraNecessarySpaceForGridColumnGroup;
	...
	_recalculateColumnWidths() {
		...
		[...this.$.header.children, ...this.$.footer.children].forEach((row) => {
			if (row.__debounceUpdateHeaderFooterRowVisibility) row.__debounceUpdateHeaderFooterRowVisibility.flush();
		});
	...
	*/
	__setVisibleCellContentAutoWidth(col, autoWidth) {
		col._allCells.filter((cell) => {
			if (this.$.items.contains(cell)) return this.__viewportRowsCache.includes(cell.parentElement);
			return true;
		}).forEach((cell) => {
		...
		if (autoWidth) this.$.scroller.setAttribute("measuring-auto-width", "");
		else this.$.scroller.removeAttribute("measuring-auto-width");
	}
	/**
	...
	*/
	__tryToRecalculateColumnWidthsIfPending() {
		if (!this.__pendingRecalculateColumnWidths) return;
	...
	*/
	__isReadyForColumnWidthCalculation() {
		if (!this._columnTree) return false;
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-column-reordering-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var ColumnReorderingMixin = (superClass) => class ColumnReorderingMixin extends superClass {
	...
	_cancelReorderForMultiTouch(e) {
		if (e.touches.length > 1) {
			...
			if (this._draggedColumn) this._onTrackEnd();
			return true;
		}
		return false;
	}
	/** @private */
	_onTouchStart(e) {
		if (this._cancelReorderForMultiTouch(e)) return;
		this._startTouchReorderTimeout = setTimeout(() => {
			this._onTrackStart({ detail: {
	...
	_onTrackEvent(e) {
		if (e.detail.state === "start") {
			...
			if (!headerCell || !headerCell._content) return;
			if (headerCell._content.contains(this.getRootNode().activeElement)) return;
			if (this.$.scroller.hasAttribute("column-resizing")) return;
			if (!this._touchDevice) this._onTrackStart(e);
	...
	_onTrackStart(e) {
		...
		const path = e.composedPath?.();
		if (path?.slice(0, Math.max(0, path.indexOf(this))).some((node) => node.draggable)) return;
		const headerCell = this._cellFromPoint(e.detail.x, e.detail.y);
		if (!headerCell || !headerCell.part.contains("header-cell")) return;
		this.toggleAttribute("reordering", true);
		this._draggedColumn = headerCell._column;
		while (this._draggedColumn.parentElement.childElementCount === 1) this._draggedColumn = this._draggedColumn.parentElement;
	...
	_onTrack(e) {
		...
		if (this._isSwapAllowed(this._draggedColumn, targetColumn) && this._isSwappableByPosition(targetColumn, e.detail.x)) {
			...
			for (let i = startIndex; i !== endIndex; i += direction) this._swapColumnOrders(this._draggedColumn, levelColumnsInOrder[i + direction]);
		}
		this._updateGhostPosition(e.detail.x, this._touchDevice ? e.detail.y - 50 : e.detail.y);
	...
	_onTrackEnd() {
		...
		this.dispatchEvent(new CustomEvent("column-reorder", { detail: { columns: this._getColumnsInOrder() } }));
	}
	/**
	...
	*/
	...
	_getCellFromElement(element) {
		if (element) {
			if (element._column) return element;
			const { parentElement } = element;
			if (parentElement?._focusButton === element) return parentElement;
		}
		return null;
	}
	/**
	...
	*/
	_resetColumnOrder() {
		if (this._columnTree === void 0) return;
		if (this._columnTree.every((level) => level.every((column, i) => i === 0 || column._order >= level[i - 1]._order))) return;
	...
	*/
	_setSiblingsReorderStatus(column, status) {
		iterateChildren(column.parentNode, (sibling) => {
			if (/column/u.test(sibling.localName) && this._isSwapAllowed(sibling, column)) sibling._reorderStatus = status;
		});
	}
	/** @protected */
	_autoScroller() {
	...
	*/
	_swapColumnOrders(column1, column2) {
		...
		[
			...
			if (row.__cells) row.__cells = row.__cells.toSorted((a, b) => a._column._order - b._column._order);
		});
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-column-resizing-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var ColumnResizingMixin = (superClass) => class ColumnResizingMixin extends superClass {
	...
	ready() {
		...
		scroller.addEventListener("touchstart", (e) => {
			if (e.touches.length > 1) scroller.removeAttribute("column-resizing");
		});
		scroller.addEventListener("touchmove", (e) => {
			...
			if (scroller.hasAttribute("column-resizing")) e.preventDefault();
		});
	...
	_onHeaderTrack(e) {
		...
		if (handle.part.contains("resize-handle")) {
			if (e.detail.state !== "start" && !this.$.scroller.hasAttribute("column-resizing")) return;
			let column = handle.parentElement._column;
			this.$.scroller.toggleAttribute("column-resizing", true);
			while (column.localName === "vaadin-grid-column-group") column = column._childColumns.slice(0).sort((a, b) => a._order - b._order).filter((column) => !column.hidden).pop();
			...
			if (targetCell.offsetWidth) {
				...
				if (targetCell.hasAttribute("frozen-to-end")) maxWidth = cellWidth + (isRTL ? eventX - cellRect.right : cellRect.left - eventX);
				else maxWidth = cellWidth + (isRTL ? cellRect.left - eventX : eventX - cellRect.right);
			...
			if (cellFrozenToEnd && this.$.table.scrollWidth > this.$.table.offsetWidth) {
				...
				if (isRTL && offset <= 0 || !isRTL && offset >= 0) this.$.table.scrollLeft += offset;
			}
			if (e.detail.state === "end") {
				this.$.scroller.toggleAttribute("column-resizing", false);
				this.dispatchEvent(new CustomEvent("column-resize", { detail: { resizedColumn: column } }));
			}
			this._resizeHandler();
		}
	}
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-data-provider-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var DataProviderMixin = (superClass) => class DataProviderMixin extends superClass {
	...
	__itemHasChildrenPathChanged(value, oldValue) {
		if (!oldValue && value === "children") return;
	...
	*/
	getItemId(item) {
		return this.itemIdPath ? get$1(this.itemIdPath, item) : item;
	}
	/**
	* @param {!GridItem} item
	* @return {boolean}
	...
	*/
	_hasChildren(item) {
		return this.itemHasChildrenPath && item && !!get$1(this.itemHasChildrenPath, item);
	}
	/** @private */
	...
	*/
	expandItem(item) {
		if (!this._isExpanded(item)) this.expandedItems = [...this.expandedItems, item];
	}
	/**
	...
	_onDataProviderPageLoaded() {
		this._debouncerApplyCachedData = Debouncer.debounce(this._debouncerApplyCachedData, timeOut.after(0), () => {
			...
			this._getRenderedRows().forEach((row) => {
				...
				if (shouldLoadAllRenderedRowsAfterPageLoad) this.__ensureRowItem(row);
			});
			this.__scrollToPendingIndexes();
			this.__dispatchPendingBodyCellFocus();
		});
		if (!this._dataProviderController.isLoading()) this._debouncerApplyCachedData.flush();
	}
	/** @private */
	...
	*/
	clearCache() {
		...
		this._hasData = false;
		this.__updateVisibleRows();
		if (!this.__virtualizer || !this.__virtualizer.size) this._dataProviderController.loadFirstPage();
	}
	/** @private */
	_pageSizeChanged(pageSize, oldPageSize) {
		this._dataProviderController.setPageSize(pageSize);
		if (oldPageSize !== void 0 && pageSize !== oldPageSize) this.clearCache();
	}
	/** @protected */
	_checkSize() {
		if (this.size === void 0 && this._flatSize === 0) console.warn("The <vaadin-grid> needs the total number of items in order to display rows, which you can specify either by setting the `size` property, or by providing it to the second argument of the `dataProvider` function `callback` call.");
	}
	/** @private */
	_dataProviderChanged(dataProvider, oldDataProvider) {
		this._dataProviderController.setDataProvider(dataProvider ? dataProvider.bind(this) : null);
		if (oldDataProvider !== void 0) this.clearCache();
	...
	_ensureFirstPageLoaded() {
		if (!this._hasData) this._dataProviderController.loadFirstPage();
	}
	/**
	...
	*/
	scrollToIndex(...indexes) {
		...
		let targetIndex;
		while (targetIndex !== (targetIndex = this._dataProviderController.getFlatIndexByPath(indexes))) this._scrollToFlatIndex(targetIndex);
		if (this._dataProviderController.isLoading()) this.__pendingScrollToIndexes = indexes;
	}
	/** @private */
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-drag-and-drop-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var DragAndDropMixin = (superClass) => class DragAndDropMixin extends superClass {
	...
	_onDragStart(e) {
		if (this.rowsDraggable) {
			let row = e.target;
			if (row.localName === "vaadin-grid-cell-content") row = row.assignedSlot.parentNode.parentNode;
			if (row.parentNode !== this.$.items) return;
			e.stopPropagation();
			...
			let rows = [row];
			if (this._isSelected(row._item)) rows = this.__getViewportRows().filter((row) => this._isSelected(row._item)).filter((row) => !this.dragFilter || this.dragFilter(this.__getRowModel(row)));
			...
			const event = new CustomEvent("grid-dragstart", { detail: {
	...
	_onDragOver(e) {
		if (this.dropMode) {
			...
			if (!this._flatSize || this.dropMode === DropMode.ON_GRID) this._dropLocation = DropLocation.EMPTY;
			else if (!row || row.parentNode !== this.$.items) if (row) return;
			...
			if (this._dropLocation === DropLocation.EMPTY) this.toggleAttribute("dragover", true);
				...
				if (row.getAttribute("dragover") !== this._dropLocation) updateStringRowStates(row, { dragover: this._dropLocation });
	...
	*/
	__onDocumentDragStart(e) {
		if (e.target.contains(this)) {
			...
			if (this.$.table.scrollHeight > 2e4) this.$.scroller.style.display = "none";
			if (isChrome) e.target.style.willChange = "transform";
			if (isSafari) this.$.items.style.flexShrink = 1;
			requestAnimationFrame(() => {
	...
	__dndAutoScroll(clientY) {
		...
		if (scrollTopDelta) {
			...
			if (scrollTop !== this.$.table.scrollTop) {
	...
	__formatDefaultTransferData(rows) {
		return rows.map((row) => {
			return getBodyRowCells(row).filter((cell) => !cell.hidden).map((cell) => cell._content.textContent.trim()).filter((content) => content).join("	");
		}).join("\n");
	}
	/** @private */
	...
	*/
	filterDragAndDrop() {
		iterateChildren(this.$.items, (row) => {
			if (!row.hidden) this._filterDragAndDrop(row, this.__getRowModel(row));
		});
	}
	/**
	* @param {!HTMLElement} row
	...
	*/
	_filterDragAndDrop(row, model) {
		...
		iterateRowCells(row, (cell) => {
			if (dragDisabled) cell._content.removeAttribute("draggable");
			else cell._content.setAttribute("draggable", true);
		});
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-dynamic-columns-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
function arrayEquals(arr1, arr2) {
	...
	for (let i = 0, l = arr1.length; i < l; i++) if (arr1[i] instanceof Array && arr2[i] instanceof Array) {
...
_columnTree: {
	...
	*/
	...
	_updateColumnTree() {
		...
		if (!arrayEquals(columnTree, this._columnTree)) this._columnTree = columnTree;
	}
	/** @private */
	_addNodeObserver() {
	...
	_checkImports() {
		[
			...
			if (this.querySelector(elementName) && !customElements.get(elementName)) console.warn(`Make sure you have imported the required module for <${elementName}> element.`);
		});
	}
	/** @protected */
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-event-context-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
	...
	*/
	getEventContext(event) {
		...
		context.section = [
			...
			"footer",
			"details"
		].find((section) => cell.part.contains(`${section}-cell`));
		if (cell._column) context.column = cell._column;
		if (context.section === "body" || context.section === "details") Object.assign(context, this.__getRowModel(cell.__parentRow));
		return context;
	}
};
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-filter-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-keyboard-navigation-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var KeyboardNavigationMixin = (superClass) => class KeyboardNavigationMixin extends superClass {
	...
	set __rowFocusMode(value) {
		[
			...
			if (value) {
				...
				if (isCell(focusable)) this[prop] = parent;
				else if (isCell(parent)) this[prop] = parent.parentElement;
	...
	_focusedCellChanged(focusedCell, oldFocusedCell) {
		if (oldFocusedCell) updatePart(oldFocusedCell, "focused-cell", false);
		if (focusedCell) updatePart(focusedCell, "focused-cell", true);
	}
	/** @private */
	_interactingChanged() {
	...
	*/
	__updateItemsFocusable() {
		...
		this._getRenderedRows().forEach((row) => {
			if (row.index === this._focusedItemIndex) if (this.__rowFocusMode) this._itemsFocusable = row;
			else {
	...
	*/
	_onKeyDown(e) {
		...
		if (this.interacting && keyGroup !== "Interaction") keyGroup = void 0;
	...
	__ensureFlatIndexInViewport(index) {
		...
		if (!targetRowInDom) this._scrollToFlatIndex(index);
		else this.__scrollIntoViewport(targetRowInDom);
	}
	/** @private */
	...
	_onNavigationKeyDown(e, key) {
		...
		switch (key) {
			...
			case "ArrowLeft":
				...
				break;
			case "Home":
				if (this.__rowFocusMode) dy = -Infinity;
				else if (e.ctrlKey) dy = -Infinity;
				else dx = -Infinity;
				break;
			case "End":
				if (this.__rowFocusMode) dy = Infinity;
		...
		if (this.__rowFocusMode && !activeRow || !this.__rowFocusMode && !activeCell) return;
		...
		if (this.__rowFocusMode) this._onRowNavigation(activeRow, dy);
		else this._onCellNavigation(activeCell, dx, dy);
	}
	/**
	...
	*/
	...
	__getIndexInGroup(row, bodyFallbackIndex) {
		...
		if (rowGroup === this.$.items) return bodyFallbackIndex ?? row.index;
	...
	*/
	_onCellNavigation(activeCell, dx, dy) {
		...
		const activeRowGroup = activeRow.parentNode;
		if (this._focusedColumnOrder === void 0) if (isCurrentCellRowDetails) this._focusedColumnOrder = 0;
		else this._focusedColumnOrder = activeCell._column._order;
		if (dstIsRowDetails) [...dstRow.children].find(isDetailsCell).focus();
		else {
			...
			if (dstOrderedColumnIndex !== orderedColumnIndex) this._focusedColumnOrder = void 0;
			...
			if (this.$.items.contains(activeCell)) {
				...
				if (this._lazyColumns) {
					if (!this.__isColumnInViewport(dstSizerCell._column)) dstSizerCell.scrollIntoView();
	...
	_onInteractionKeyDown(e, key) {
		...
		if (this.interacting !== wantInteracting && cell !== null) if (wantInteracting) {
		...
		if (key === "Escape") this._hideTooltip(true);
	}
	/** @private */
	_predictFocusStepTarget(srcElement, step) {
		...
		if (focusStepTarget && !this.__isHorizontallyInViewport(focusStepTarget)) {
			...
			if (firstVisibleColumn) {
				if (focusStepTarget === this._headerFocusable) focusStepTarget = firstVisibleColumn._headerCell;
				...
				} else if (focusStepTarget === this._footerFocusable) focusStepTarget = firstVisibleColumn._footerCell;
			}
		}
	...
	_onTabKeyDown(e) {
		...
		focusTarget.focus();
		if (focusTarget !== this.$.table && focusTarget !== this.$.focusexit) e.preventDefault();
		this.toggleAttribute("navigating", true);
	}
	/** @private */
	_onSpaceKeyDown(e) {
		...
		if (isElementRow || !element._content || !element._content.firstElementChild) this.dispatchEvent(new CustomEvent(isElementRow ? "row-activate" : "cell-activate", { detail: { model: this.__getRowModel(isElementRow ? element : element.parentElement) } }));
	}
	/** @private */
	_onKeyUp(e) {
		if (!/^( |SpaceBar)$/u.test(e.key) || this.interacting) return;
		e.preventDefault();
	...
	*/
	_onFocusIn(e) {
		...
		if (rootTarget === this.$.table || rootTarget === this.$.focusexit) {
			if (!this._isMousedown) this._predictFocusStepTarget(rootTarget, rootTarget === this.$.table ? 1 : -1).focus();
			this._setInteracting(false);
	...
	*/
	...
	_onContentFocusIn(e) {
		...
		if (!cell && !this.__rowFocusMode) return;
		this._detectInteracting(e);
		if (section && (cell || row)) {
			this._activeRowGroup = section;
			if (section === this.$.header) this._headerFocusable = this.__getFocusable(row, cell);
			else if (section === this.$.items) {
				this._itemsFocusable = this.__getFocusable(row, cell);
				this._focusedItemIndex = row.index;
			} else if (section === this.$.footer) this._footerFocusable = this.__getFocusable(row, cell);
			if (cell) {
				const context = this.getEventContext(e);
				this.__pendingBodyCellFocus = this.loading && context.section === "body";
				if (!this.__pendingBodyCellFocus && cell !== this.$.emptystatecell) cell.dispatchEvent(new CustomEvent("cell-focus", {
					bubbles: true,
					composed: true,
					detail: { context }
				}));
				this._focusedCell = cell._focusButton || cell;
				if (isKeyboardActive() && e.target === cell) this._showTooltip(e);
			} else this._focusedCell = null;
		}
	}
	/**
	* @private
	*/
	__dispatchPendingBodyCellFocus() {
		if (this.__pendingBodyCellFocus && this.shadowRoot.activeElement === this._itemsFocusable) this._itemsFocusable.dispatchEvent(new Event("focusin", {
			bubbles: true,
			composed: true
		}));
	}
	...
	*/
	_updateGridSectionFocusTarget(focusTarget) {
		...
		focusTarget.tabIndex = this.interacting && section === this._activeRowGroup ? -1 : 0;
	}
	/** @protected */
	_preventScrollerRotatingCellFocus() {
		if (this._activeRowGroup !== this.$.items) return;
		this.__preventScrollerRotatingCellFocusDebouncer = Debouncer.debounce(this.__preventScrollerRotatingCellFocusDebouncer, animationFrame, () => {
			...
			if (this._getRenderedRows().some((row) => row.index === this._focusedItemIndex)) {
				this.__updateItemsFocusable();
				if (isItemsRowGroupActive && !this.__rowFocusMode) this._focusedCell = this._itemsFocusable;
	...
	*/
	...
	_resetKeyboardNavigation() {
		if (!this.$ && this.performUpdate) this.performUpdate();
		["header", "footer"].forEach((section) => {
			if (!this.__isValidFocusable(this[`_${section}Focusable`])) {
				...
				if (firstVisibleRow && firstVisibleCell) this[`_${section}Focusable`] = this.__getFocusable(firstVisibleRow, firstVisibleCell);
			}
		});
	...
	*/
	_scrollHorizontallyToCell(dstCell) {
		if (dstCell.hasAttribute("frozen") || dstCell.hasAttribute("frozen-to-end") || isDetailsCell(dstCell)) return;
		...
		for (let i = dstCellIndex - 1; i >= 0; i--) {
			...
			if (cell.hasAttribute("hidden") || isDetailsCell(cell)) continue;
		...
		if (dstCellRect.left < leftBoundary) this.$.table.scrollLeft += dstCellRect.left - leftBoundary;
		if (dstCellRect.right > rightBoundary) this.$.table.scrollLeft += dstCellRect.right - rightBoundary;
	}
	/**
	...
	*/
	_getGridEventLocation(e) {
		...
		return {
			section: tableIndex >= 1 ? path[tableIndex - 1] : null,
			row: tableIndex >= 2 ? path[tableIndex - 2] : null,
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-resize-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var ResizeMixin = (superClass) => class extends superClass {
	...
	ready() {
		...
		const resizeObserver = new ResizeObserver((entries) => {
			if (entries.findLast(({ target }) => target === this)) this.__hostVisible = this.checkVisibility();
			const tableEntry = entries.findLast(({ target }) => target === this.$.table);
			if (tableEntry) this.__tableRect = tableEntry.contentRect;
			const headerEntry = entries.findLast(({ target }) => target === this.$.header);
			if (headerEntry) this.__headerRect = headerEntry.contentRect;
			const itemsEntry = entries.findLast(({ target }) => target === this.$.items);
			if (itemsEntry) this.__itemsRect = itemsEntry.contentRect;
			const footerEntry = entries.findLast(({ target }) => target === this.$.footer);
			if (footerEntry) this.__footerRect = footerEntry.contentRect;
		});
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-row-details-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var RowDetailsMixin = (superClass) => class RowDetailsMixin extends superClass {
	...
	_rowDetailsRendererChanged(rowDetailsRenderer) {
		...
		if (this._columnTree) this._getRenderedRows().forEach((row) => {
			if (!row.querySelector("[part~=details-cell]")) {
				this.__initRow(row, this._columnTree[this._columnTree.length - 1]);
				this.__updateRow(row);
				return;
			}
			if (row.hasAttribute("details-opened")) this.__updateRow(row);
		});
	}
	/** @private */
	_detailsOpenedItemsChanged(_detailsOpenedItems, rowDetailsRenderer) {
		this._getRenderedRows().forEach((row) => {
			if (row.hasAttribute("details-opened") !== !!(rowDetailsRenderer && this._isDetailsOpened(row._item))) this.__updateRow(row);
		});
	}
	/**
	* @param {!HTMLElement} cell
	...
	*/
	openItemDetails(item) {
		if (!this._isDetailsOpened(item)) this.detailsOpenedItems = [...this.detailsOpenedItems, item];
	}
	/**
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-scroll-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var ScrollMixin = (superClass) => class ScrollMixin extends superClass {
		...
		this.$.items.addEventListener("focusin", (e) => {
			...
			if (row) {
				if (!this._isMousedown) {
					...
					const scrollTarget = row.clientHeight > viewportHeight ? e.target : row;
					this.__scrollIntoViewport(scrollTarget);
				}
				if (!this.$.table.contains(e.relatedTarget)) this.$.table.dispatchEvent(new CustomEvent("virtualizer-element-focused", { detail: { element: row } }));
			}
		});
	...
	*/
	scrollToColumn(indexOrColumn) {
		...
		if (column.frozen || column.frozenToEnd) return;
	...
	__scrollToPendingColumn() {
		if (this.__pendingScrollToColumn !== void 0) {
	...
	*/
	__scrollIntoViewport(element) {
		...
		const footerTop = this.$.footer.getBoundingClientRect().top;
		const headerBottom = this.$.header.getBoundingClientRect().bottom;
		if (elementBottom > footerTop) this.$.table.scrollTop += elementBottom - footerTop;
		else if (elementTop < headerBottom) this.$.table.scrollTop -= headerBottom - elementTop;
	}
	/** @private */
	_scheduleScrolling() {
		if (!this._scrollingFrame) this._scrollingFrame = requestAnimationFrame(() => this.$.scroller.toggleAttribute("scrolling", true));
	...
	_afterScroll() {
		...
		if (!this.hasAttribute("navigating")) this._hideTooltip(true);
	...
	__updateColumnsBodyContentHidden() {
		if (!this._columnTree || !this._areSizerCellsAssigned()) return;
			...
			if (column._bodyContentHidden !== bodyContentHidden) {
				...
				column._cells.forEach((cell) => {
					if (cell !== column._sizerCell) {
						if (bodyContentHidden) cell.remove();
		...
		if (bodyContentHiddenChanged) this._frozenCellsChanged();
		if (this._lazyColumns) {
	...
	__getColumnEnd(column) {
		if (!column) return this.__isRTL ? this.$.table.clientWidth : 0;
	...
	*/
	__isColumnInViewport(column) {
		if (column.frozen || column.frozenToEnd) return true;
	...
	__columnRenderingChanged(_columnTree, columnRendering) {
		if (columnRendering === "eager") this.$.scroller.removeAttribute("column-rendering");
		else this.$.scroller.setAttribute("column-rendering", columnRendering);
		this.__updateColumnsBodyContentHidden();
	}
	/** @protected */
	_frozenCellsChanged() {
	...
	_updateFrozenColumn() {
		...
		for (let i = 0; i < columnsRow.length; i++) {
			...
			if (firstFrozenToEnd === void 0 && col.frozenToEnd && !col.hidden) firstFrozenToEnd = i;
			if (col.frozen && !col.hidden) lastFrozen = i;
		}
		if (lastFrozen !== void 0) columnsRow[lastFrozen]._lastFrozen = true;
		if (firstFrozenToEnd !== void 0) columnsRow[firstFrozenToEnd]._firstFrozenToEnd = true;
		this.__updateColumnsBodyContentHidden();
	}
	/** @private */
	__updateHorizontalScrollPosition() {
		...
		if (this._lazyColumns && this._areSizerCellsAssigned()) {
			...
			transformFrozenToEndBody = `translate(${remaining + (this.__getColumnStart(firstFrozenToEndColumn) - lastVisibleColumnEnd) + this.__lazyColumnsStart}px, 0)`;
		}
		this._frozenToEndCells.forEach((cell) => {
			if (this.$.items.contains(cell)) cell.style.transform = transformFrozenToEndBody;
			else cell.style.transform = transformFrozenToEnd;
		});
		const focusedRow = this.shadowRoot.querySelector("[part~='row']:focus");
		if (focusedRow) this.__updateRowScrollPositionProperty(focusedRow);
		const lastHeaderRow = this.$.header.querySelector("[part~='last-header-row']");
		if (lastHeaderRow) this.__updateRowScrollPositionProperty(lastHeaderRow);
		const firstFooterRow = this.$.footer.querySelector("[part~='first-footer-row']");
		if (firstFooterRow) this.__updateRowScrollPositionProperty(firstFooterRow);
	}
	/**
	...
	*/
	__updateRowScrollPositionProperty(row) {
		if (row instanceof HTMLTableRowElement === false) return;
		const newValue = `${this.__horizontalScrollPosition}px`;
		if (row.style.getPropertyValue("--_grid-horizontal-scroll-position") !== newValue) row.style.setProperty("--_grid-horizontal-scroll-position", newValue);
	}
	/** @private */
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-selection-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var SelectionMixin = (superClass) => class SelectionMixin extends superClass {
	...
	*/
	__isItemSelectable(item) {
		if (!this.isItemSelectable || !item) return true;
	...
	*/
	selectItem(item) {
		if (!this._isSelected(item)) this.selectedItems = [...this.selectedItems, item];
	}
	/**
 ...
 /** @internal to not document it in CEM */
	...
	__selectedItemsChanged() {
		this._getRenderedRows().forEach((row) => {
			if (row.hasAttribute("selected") !== this._isSelected(row._item) || row.hasAttribute("nonselectable") !== !this.__isItemSelectable(row._item)) this.__updateRow(row);
		});
	}
	/** @private */
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-sort-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var SortMixin = (superClass) => class SortMixin extends superClass {
	...
	*/
	...
	__updateSortOrders() {
		...
		if (activeSorters.length > 1) activeSorters.forEach((sorter, index) => {
			sorter._order = index;
		});
	}
	/** @private */
	__updateSorter(sorter, shiftClick, fromSorterClick) {
		if (!sorter.direction && !this._sorters.includes(sorter)) return;
		sorter._order = null;
		const restSorters = this._sorters.filter((s) => s !== sorter);
		if (this.multiSort && (!this.multiSortOnShiftClick || !fromSorterClick) || this.multiSortOnShiftClick && shiftClick) if (this.multiSortPriority === "append") this._sorters = [...restSorters, sorter];
		else this._sorters = [sorter, ...restSorters];
	...
	__applySorters() {
		...
		if (this.dataProvider && this.isAttached && JSON.stringify(this._previousSorters) !== JSON.stringify(this._mapSorters())) this.__debounceClearCache();
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-styling-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
cellPartNameGenerator: {
	...
	*/
	generateCellPartNames() {
		iterateChildren(this.$.items, (row) => {
			if (!row.hidden) this._generateCellPartNames(row, this.__getRowModel(row));
		});
	}
	/** @private */
	_generateCellPartNames(row, model) {
		iterateRowCells(row, (cell) => {
			if (cell.__generatedParts) cell.__generatedParts.forEach((partName) => {
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var GridMixin = (superClass) => class extends ColumnAutoWidthMixin(ArrayDataProviderMixin(DataProviderMixin(DynamicColumnsMixin(ActiveItemMixin(ScrollMixin(SelectionMixin(SortMixin(RowDetailsMixin(KeyboardNavigationMixin(A11yMixin(FilterMixin(ColumnReorderingMixin(ColumnResizingMixin(EventContextMixin(DragAndDropMixin(StylingMixin(TabindexMixin(ResizeMixin(superClass))))))))))))))))))) {
	...
	_getRowContainingNode(node) {
		...
		if (!content) return;
		return content.assignedSlot.parentElement.parentElement;
	}
	/** @protected */
	...
	updated(props) {
		...
		if (props.has("__headerRect") || props.has("__footerRect") || props.has("__itemsRect")) setTimeout(() => this.__updateMinHeight());
	...
	_flatSizeChanged(flatSize, virtualizer, hasData, columnTree) {
		if (virtualizer && hasData && columnTree) {
			...
			if (flatSize < previousSize) virtualizer.update(flatSize - 1, flatSize - 1);
			if (cellCoordinates && cell.parentElement.hidden) this.__focusBodyCell(cellCoordinates);
			this._resetKeyboardNavigation();
		}
	}
	/** @private */
	__createVirtualizerElements(count) {
		...
		for (let i = 0; i < count; i++) {
			...
			if (this._columnTree) this.__initRow(row, this._columnTree[this._columnTree.length - 1], "body", false, true);
			rows.push(row);
		}
		if (this._columnTree) this._columnTree[this._columnTree.length - 1].forEach((c) => {
			if (c.isConnected && c._cells) c._cells = [...c._cells];
		});
		return rows;
	}
	/** @private */
	_createCell(tagName, column) {
		const slotName = `vaadin-grid-cell-content-${this._contentIndex = this._contentIndex + 1 || 0}`;
		...
		if (!isAndroid && !isIOS) {
			cell.addEventListener("mouseenter", (event) => {
				if (!this.$.scroller.hasAttribute("scrolling")) this._showTooltip(event);
			});
		...
		cellContent.addEventListener("mousedown", () => {
			if (isChrome) {
				const mouseUpListener = (event) => {
					...
					if (!contentContainsFocusedElement && mouseUpWithinCell) cell.focus({ preventScroll: true });
					document.removeEventListener("mouseup", mouseUpListener, true);
				};
				document.addEventListener("mouseup", mouseUpListener, true);
			} else setTimeout(() => {
				if (!cellContent.contains(this.getRootNode().activeElement)) cell.focus({ preventScroll: true });
			});
		});
		return cell;
	}
	...
	*/
	__initRow(row, columns, section = "body", isColumnRow = false, noNotify = false) {
		...
		columns.filter((column) => !column.hidden).toSorted((a, b) => a._order - b._order).forEach((column, index, cols) => {
			let cell;
			if (section === "body") {
				if (!column._cells) column._cells = [];
				cell = column._cells.find((cell) => cell._vacant);
				if (!cell) {
					cell = this._createCell("td", column);
					if (column._onCellKeyDown) cell.addEventListener("keydown", column._onCellKeyDown.bind(column));
				...
				if (!column._bodyContentHidden || isSizerRow) row.appendChild(cell);
				if (isSizerRow) column._sizerCell = cell;
				if (index === cols.length - 1 && this.rowDetailsRenderer) {
					if (!this._detailsCells) this._detailsCells = [];
				...
				if (!noNotify) column._cells = [...column._cells];
			} else {
				...
				if (isColumnRow || column.localName === "vaadin-grid-column-group") {
					...
					if (!column._emptyCells) column._emptyCells = [];
					cell = column._emptyCells.find((cell) => cell._vacant) || this._createCell(tagName);
					cell._column = column;
					row.appendChild(cell);
					if (column._emptyCells.indexOf(cell) === -1) column._emptyCells.push(cell);
				}
				updatePart(cell, "cell", true);
				updatePart(cell, `${section}-cell`, true);
			}
			if (!cell._content.parentElement) contentsFragment.appendChild(cell._content);
			cell._vacant = false;
			cell._column = column;
		});
		if (section !== "body") this.__debounceUpdateHeaderFooterRowVisibility(row);
	...
	*/
	__updateHeaderFooterRowVisibility(row) {
		...
		const visibleRowCells = Array.from(row.children).filter((cell) => {
			...
			if (column._emptyCells && column._emptyCells.indexOf(cell) > -1) return false;
			if (row.parentElement === this.$.header) {
				if (column.headerRenderer) return true;
				if (column.header === null) return false;
				if (column.path || column.header !== void 0) return true;
			} else if (column.footerRenderer) return true;
			return false;
		});
	...
	*/
	__updateRowLoading(row, loading) {
		...
		if (loading) this._generateCellPartNames(row);
	}
	/**
	* @param {!HTMLElement} row
	* @private
	*/
	__updateRow(row) {
		...
		if (item) this.__updateRowLoading(row, false);
		else {
		...
		iterateChildren(row, (cell) => {
			if (cell._column && !cell._column.isConnected) return;
			if (cell._renderer) {
	...
	*/
	_showTooltip(event) {
		if (this._tooltipController.node?.isConnected) {
			const target = event.target;
			if (!this.__isCellFullyVisible(target)) return;
	...
	__isCellFullyVisible(cell) {
		if (cell.hasAttribute("frozen") || cell.hasAttribute("frozen-to-end")) return true;
	...
	__updateHeaderAndFooter() {
		(this._columnTree || []).forEach((level) => {
			level.forEach((column) => {
				if (column._renderHeaderAndFooter) column._renderHeaderAndFooter();
			});
		});
	}
	/** @protected */
	__updateVisibleRows(start, end) {
		if (this.__virtualizer) this.__virtualizer.update(start, end);
	}
	/** @private */
	__updateMinHeight() {
		const rowHeight = 36;
...
//#endregion
//#region node_modules/@vaadin/grid/src/vaadin-grid.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
//#endregion
export { gridStyles as n, Grid as t };

//# sourceMappingURL=vaadin-grid-Ba7vVF3C.js.map