...
import { n as get } from "./polylit-mixin-BwU08z1T.js";
import { t as DirMixin } from "./dir-mixin-rxMTFGYq.js";
import { Debouncer } from "./@vaadin_component-base_src_debounce__js.js";
import { updateCellState, updatePart } from "./@vaadin_grid_src_vaadin-grid-helpers__js.js";
//#region node_modules/@vaadin/grid/src/vaadin-grid-column-mixin.js
/**
* @license
...
var ColumnBaseMixin = (superClass) => class ColumnBaseMixin extends superClass {
			...
			*/
			resizable: {
				...
				value() {
					...
					if (parent?.localName === "vaadin-grid-column-group") return parent.resizable || false;
					return false;
				}
			},
			/**
	...
	*/
	_findHostGrid() {
		let el = this;
		while (el && !/^vaadin.*grid(-pro)?$/u.test(el.localName)) el = el.assignedSlot ? el.assignedSlot.parentNode : el.parentNode;
		return el || void 0;
	}
	/** @protected */
	...
	_frozenChanged(frozen) {
		...
		if (this._grid && this._grid._frozenCellsChanged) this._grid._frozenCellsChanged();
	}
	/** @private */
	_frozenToEndChanged(frozenToEnd) {
		if (this.parentElement && this.parentElement._columnPropChanged) this.parentElement._columnPropChanged("frozenToEnd", frozenToEnd);
		this._allCells.forEach((cell) => {
			if (this._grid && cell.parentElement === this._grid.$.sizer) return;
	...
	_firstFrozenToEndChanged(firstFrozenToEnd) {
		...
		if (this.parentElement && this.parentElement._columnPropChanged) this.parentElement._firstFrozenToEnd = firstFrozenToEnd;
	}
	/** @private */
	...
	*/
	...
	_resizableChanged(resizable, headerCell) {
		if (resizable === void 0 || headerCell === void 0) return;
		if (headerCell) [headerCell].concat(this._emptyCells).forEach((cell) => {
			if (cell) {
	...
	_textAlignChanged(textAlign) {
		if (textAlign === void 0 || this._grid === void 0) return;
		if ([
			"start",
			"end",
			"center"
	...
	_hiddenChanged(hidden) {
		...
		if (!!hidden !== !!this._previousHidden && this._grid) {
			...
			if (this._grid._debounceUpdateFrozenColumn) this._grid._debounceUpdateFrozenColumn();
			if (this._grid._resetKeyboardNavigation) this._grid._resetKeyboardNavigation();
		}
	...
	_runRenderer(renderer, cell, model) {
		...
		if (!(isVisibleBodyCell || renderer === this._headerRenderer || renderer === this._footerRenderer)) return;
		const args = [cell._content, this];
		if (isVisibleBodyCell) args.push(model);
	...
	*/
	_renderHeaderCellContent(headerRenderer, headerCell) {
		...
		if (this._grid && headerCell.parentElement) this._grid.__debounceUpdateHeaderFooterRowVisibility(headerCell.parentElement);
	}
	/** @protected */
	...
	*/
	_renderFooterCellContent(footerRenderer, footerCell) {
		...
		if (this._grid && footerCell.parentElement) this._grid.__debounceUpdateHeaderFooterRowVisibility(footerCell.parentElement);
	}
	/** @protected */
	...
	*/
	_computeHeaderRenderer(headerRenderer, header) {
		...
		if (header !== void 0 && header !== null) return this.__textHeaderRenderer;
...
//#endregion
export { GridColumnMixin as n, ColumnBaseMixin as t };

//# sourceMappingURL=vaadin-grid-column-mixin-DE4xLgqG.js.map