import { animationFrame, idlePeriod, microTask, timeOut } from "./@vaadin_component-base_src_async__js.js";
import { Debouncer, enqueueDebouncer, flush } from "./@vaadin_component-base_src_debounce__js.js";
import { isSafari } from "./@vaadin_component-base_src_browser-utils__js.js";
//#region node_modules/@vaadin/component-base/src/iron-list-core.js
/**
* @license
...
var ironList = {
	...
	get firstVisibleIndex() {
		...
		if (idx == null) {
			...
			idx = this._iterateItems((pidx, vidx) => {
				...
				if (physicalOffset > this._scrollPosition) return vidx;
			}) || 0;
	...
	get lastVisibleIndex() {
		...
		if (idx == null) {
			...
			this._iterateItems((pidx, vidx) => {
				if (physicalOffset < this._scrollBottom) idx = vidx;
				physicalOffset += this._getPhysicalSizeIncrement(pidx);
			});
	...
	*/
	_increasePoolIfNeeded(count) {
		const delta = this._clamp(this._physicalCount + count, DEFAULT_PHYSICAL_COUNT, this._virtualCount - this._virtualStart) - this._physicalCount;
		let nextIncrease = Math.round(this._physicalCount * .5);
		if (delta < 0) return;
		if (delta > 0) {
			const ts = window.performance.now();
			[].push.apply(this._physicalItems, this._createPool(delta));
			for (let i = 0; i < delta; i++) this._physicalSizes.push(0);
		...
		if (this._virtualEnd >= this._virtualCount - 1 || nextIncrease === 0) {} else if (!this._isClientFull()) this._debounce("_increasePoolIfNeeded", this._increasePoolIfNeeded.bind(this, nextIncrease), microTask);
		else if (this._physicalSize < this._optPhysicalSize) this._debounce("_increasePoolIfNeeded", this._increasePoolIfNeeded.bind(this, this._clamp(Math.round(50 / this._templateCost), 1, nextIncrease)), idlePeriod);
	},
	/**
	...
	*/
	_itemsChanged(change) {
		if (change.path === "items") {
			...
			this._firstVisibleIndexVal = null;
			this._lastVisibleIndexVal = null;
			if (!this._physicalItems) this._physicalItems = [];
			if (!this._physicalSizes) this._physicalSizes = [];
	...
	*/
	_iterateItems(fn, itemSet) {
		...
		if (arguments.length === 2 && itemSet) for (i = 0; i < itemSet.length; i++) {
			...
			for (; pidx < this._physicalCount; pidx++, vidx++) if ((rtn = fn.call(this, pidx, vidx)) != null) return rtn;
			for (pidx = 0; pidx < this._physicalStart; pidx++, vidx++) if ((rtn = fn.call(this, pidx, vidx)) != null) return rtn;
		}
	},
	/**
	...
	*/
	scrollToIndex(idx) {
		...
		if (!this._isIndexRendered(idx) || idx >= this._maxVirtualStart) this._virtualStart = idx - 1;
		this._assignModels();
		this._updateMetrics();
	...
	_resizeHandler() {
		this._debounce("_render", () => {
			...
			if (this._isVisible) {
				...
				this._resetAverage();
				this._render();
			} else this.toggleScrollListener(false);
		}, animationFrame);
	},
...
//#endregion
//#region node_modules/@vaadin/component-base/src/virtualizer-iron-list-adapter.js
/**
* @license
...
var IronListAdapter = class {
	constructor({ createElements, updateElement, scrollTarget, scrollContainer, reorderElements, elementsContainer, __disableHeightPlaceholder }) {
		...
		new ResizeObserver(([{ contentRect }]) => {
			...
			if (!isHidden && this.__scrollTargetHidden && this.scrollTarget.scrollTop !== this._scrollPosition) this.scrollTarget.scrollTop = this._scrollPosition;
			this.__scrollTargetHidden = isHidden;
		}).observe(this.scrollTarget);
	...
	scrollToIndex(index) {
		...
		if (this._physicalCount <= 3) this.flush();
		...
		} else if (targetVirtualIndex < visibleElementCount) if (index < OFFSET_ADJUST_MIN_THRESHOLD) {
		...
		if (this.adjustedFirstVisibleIndex !== index && this._scrollTop < this._maxScrollTop && !this.grid) this._scrollTop -= this.__getIndexScrollOffset(index) || 0;
		this._scrollHandler();
		if (this.__hasPlaceholders()) this.__pendingScrollToIndex = index;
	}
	flush() {
		...
		flush();
		this._scrollHandler();
		if (this.__fixInvalidItemPositioningDebouncer) this.__fixInvalidItemPositioningDebouncer.flush();
		if (this.__scrollReorderDebouncer) this.__scrollReorderDebouncer.flush();
		if (this.__debouncerWheelAnimationFrame) this.__debouncerWheelAnimationFrame.flush();
	}
	hostConnected() {
		if (this.scrollTarget.offsetParent && this.scrollTarget.scrollTop !== this._scrollPosition) this.scrollTarget.scrollTop = this._scrollPosition;
	}
	...
	*/
	...
	__getBorderBoxHeight(el) {
		...
		if (style.boxSizing === "border-box") return itemHeight;
	...
	*/
	__afterElementsUpdated(updatedElements) {
		if (!this.__disableHeightPlaceholder) updatedElements.forEach((el) => {
		...
		if (this.__pendingScrollToIndex !== void 0 && !this.__hasPlaceholders()) this.scrollToIndex(this.__pendingScrollToIndex);
	}
	...
	*/
	__restoreScrollOffset(index, offsetBefore) {
		...
		if (offsetBefore !== void 0 && offsetAfter !== void 0) this._scrollTop += offsetBefore - offsetAfter;
	}
	get size() {
		return this.__size;
	}
	set size(size) {
		...
		if (this.__fixInvalidItemPositioningDebouncer) this.__fixInvalidItemPositioningDebouncer.cancel();
		if (this._debouncers && this._debouncers._increasePoolIfNeeded) this._debouncers._increasePoolIfNeeded.cancel();
		...
		if (!this._isVisible) this._assignModels();
	...
	/** @private */
	get items() {
		return { length: Math.min(this.size, MAX_VIRTUAL_COUNT) };
	}
	/** @private */
	get offsetHeight() {
	...
	__getFocusedElement(visibleElements = this.__getVisibleElements()) {
		...
		while (node && !visibleElements.includes(node)) node = node.assignedSlot || node.parentNode || node.host;
		return node;
	}
	/** @private */
	...
	__onElementFocused(e) {
		if (!this.reorderElements) return;
		const focusedElement = e.detail.element;
		if (!focusedElement) return;
		const visibleElements = this.__getVisibleElements();
		if (this.__previousFocusableSiblingMissing(focusedElement, visibleElements) || this.__nextFocusableSiblingMissing(focusedElement, visibleElements)) this.flush();
	...
	_scrollHandler() {
		...
		if (delta) {
			...
			if (!this.__overscrollDebouncer?.isActive()) this.scrollTarget.style.overscrollBehavior = "none";
		...
		if (this.reorderElements) this.__scrollReorderDebouncer = Debouncer.debounce(this.__scrollReorderDebouncer, timeOut.after(this.timeouts.SCROLL_REORDER), () => this.__reorderElements());
		if (this._scrollPosition === 0 && this.firstVisibleIndex !== 0 && Math.abs(delta) > 0) this.scrollToIndex(0);
	}
	/** @override */
	_resizeHandler() {
	...
	*/
	_increasePoolIfNeeded(count) {
		if (this._physicalCount > 2 && this._physicalAverage > 0 && count > 0) {
			const missingItemCount = Math.ceil(this._optPhysicalSize / this._physicalAverage) - this._physicalCount;
			super._increasePoolIfNeeded(Math.max(count, Math.min(100, missingItemCount)));
		} else super._increasePoolIfNeeded(count);
	}
	/**
	* An optimal physical size such that we will have enough physical items
	* to fill up the viewport and recycle when the user scrolls.
	...
	*/
	get _optPhysicalSize() {
		...
		if (optPhysicalSize <= 0 || this.__hasPlaceholders()) return optPhysicalSize;
	...
	*/
	__getItemHeightBuffer() {
		if (this._physicalCount === 0) return 0;
		const bufferZoneHeight = Math.ceil(this._viewportHeight * (this._maxPages - 1) / 2);
		const maxItemHeight = Math.max(...this._physicalSizes);
		if (maxItemHeight > Math.min(...this._physicalSizes)) return Math.max(0, maxItemHeight - bufferZoneHeight);
		return 0;
	}
	...
	__reorderElements() {
		...
		else if (delta < 0) for (let i = visibleElements.length + delta; i < visibleElements.length; i++) this.elementsContainer.insertBefore(visibleElements[i], visibleElements[0]);
		if (isSafari) {
	...
	_adjustVirtualIndexOffset(delta) {
		...
		if (this._virtualCount >= this.size) this._vidxOffset = 0;
			...
			if (fvi !== void 0) this.__restoreScrollOffset(fvi, fviOffsetBefore);
		}
	}
};
Object.setPrototypeOf(IronListAdapter.prototype, ironList);
//#endregion
//#region node_modules/@vaadin/component-base/src/virtualizer.js
/**
* @license
...
//#endregion
export { Virtualizer as t };

//# sourceMappingURL=virtualizer-DLVbY-3F.js.map