...
var PROJJSONBuilderBase = class {
	...
	static convertAxis(node) {
		...
		} else direction = node[2] ? node[2].toLowerCase() : "unknown";
...
function parse(code) {
	...
	} else if (!code.projName) return wkt_parser_default(code);
	else return code;
}
//#endregion
...
function get(name) {
	...
	if (typeof names$32[n] !== "undefined" && projStore[names$32[n]]) return projStore[names$32[n]];
	n = getNormalizedProjName(n);
	if (n in names$32 && projStore[names$32[n]]) return projStore[names$32[n]];
}
function start() {
	projs.forEach(add);
}
...
*/
function forward$30(ll, accuracy) {
	accuracy = accuracy || 5;
...
Point.prototype.toMGRS = function(accuracy) {
	return forward$30([this.x, this.y], accuracy);
};
//#endregion
...
function inverse$25(p) {
	...
	if (this.sphere) {
		...
		if (Math.abs(this.coslat0) < 1e-10) if (this.lat0 > 0) lon = adjust_lon_default(this.long0 + Math.atan2(p.x, -1 * p.y));
...
var import_geographiclib_geodesic_min = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(cb) {
		var geodesic = {};
		geodesic.Constants = {};
		geodesic.Math = {};
		...
		(function(m) {
			...
			m.sincosd = function(x) {
				var d = x % 360, r, q = Math.round(d / 90), s, c, sinx, cosx;
				d -= 90 * q;
				r = d * this.degree;
				s = Math.sin(r);
				c = Math.cos(r);
		...
		(function(g, l, p, m, c) {
			...
			g.A1m1f = function(eps) {
				...
				return (m.polyval(p, A1m1f_coeff, 0, m.sq(eps)) / A1m1f_coeff[p + 1] + eps) / (1 - eps);
			};
			C1f_coeff = [
				-1,
				6,
			...
			g.A2m1f = function(eps) {
				...
				return (m.polyval(p, A2m1f_coeff, 0, m.sq(eps)) / A2m1f_coeff[p + 1] - eps) / (1 + eps);
			};
			C2f_coeff = [
				1,
				2,
			...
			g.Geodesic.prototype.InverseStart = function(sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, slam12, clam12, C1a, C2a) {
				...
				if (shortline && ssig12 < this._etol2) {
					...
					if (y > -tol1_ && x > -1 - xthresh_) if (this.f >= 0) {
			...
			g.Geodesic.prototype.Lambda12 = function(sbet1, cbet1, dn1, sbet2, cbet2, dn2, salp1, calp1, slam120, clam120, diffp, C1a, C2a, C3a) {
				...
				if (diffp) if (vals.calp2 === 0) vals.dlam12 = -2 * this._f1 * dn1 / sbet1;
				else {
			...
			g.Geodesic.prototype.InverseInt = function(lat1, lon1, lat2, lon2, outmask) {
				...
				if (outmask & g.AREA) {
					...
					if (!meridian && comg12 > -.7071 && sbet2 - sbet1 < 1.75) {
						domg12 = 1 + comg12;
						dbet1 = 1 + cbet1;
						dbet2 = 1 + cbet2;
				...
				if (swapp < 0) {
					...
					if (outmask & g.GEODESICSCALE) [vals.M21, vals.M12] = [vals.M12, vals.M21];
				}
				salp1 *= swapp * lonsign;
				calp1 *= swapp * latsign;
				salp2 *= swapp * lonsign;
		...
		(function(p, g, m, a) {
			"use strict";
			var transit = function(lon1, lon2) {
				...
				} else if (area.Sum() >= area0) area.Add(-area0);
...
function init$8() {
	...
	this.g = new import_geographiclib_geodesic_min.Geodesic.Geodesic(this.a, this.es / (1 + Math.sqrt(1 - this.es)));
}
...
function inverse$7(p) {
	...
	if (this.sphere) {
		...
		if (Math.abs(rh) <= 1e-10) lat = this.lat0;
			...
			if (Math.abs(con) <= 1e-10) if (this.lat0 >= 0) lon = adjust_lon_default(this.long0 + Math.atan2(p.x, -p.y));
			else lon = adjust_lon_default(this.long0 - Math.atan2(-p.x, p.y));
