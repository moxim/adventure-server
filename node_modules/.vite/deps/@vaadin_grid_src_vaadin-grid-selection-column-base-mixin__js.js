...
//#region node_modules/@vaadin/grid/src/vaadin-grid-selection-column-base-mixin.js
/**
* @license
...
var GridSelectionColumnBaseMixin = (superClass) => class GridSelectionColumnBaseMixin extends superClass {
	...
	*/
	__onSelectAllCheckboxChange(e) {
		if (this._indeterminate || e.currentTarget.checked) this._selectAll();
		else this._deselectAll();
	}
	/** @private */
	__onGridInteraction(e) {
		this._shiftKeyDown = e.shiftKey;
		if (this.autoSelect) this._grid.$.scroller.toggleAttribute("range-selecting", this._shiftKeyDown);
	}
	/**
	...
	*/
	...
	__onCellTrack(event) {
		...
		if (event.detail.state === "start") {
			if (this.__multiTouchActive) return;
			const dragStartRow = this._grid._getRenderedRows().find((row) => row.contains(event.currentTarget.assignedSlot));
			...
			if (this.__dragStartItem) this.__toggleItem(this.__dragStartItem, this.__selectOnDrag);
			setTimeout(() => {
	...
	/** @private */
	__onCellMouseDown(e) {
		if (this.dragSelect) e.preventDefault();
	}
	/** @private */
	__onTouchStart(e) {
		...
		} else this.__multiTouchActive = false;
	}
	/** @private */
	__onCellClick(e) {
		if (this.__dragStartIndex !== void 0) e.preventDefault();
	}
	/** @private */
	_onCellKeyDown(e) {
		const target = e.composedPath()[0];
		if (e.keyCode !== 32) return;
		if (target === this._headerCell) if (this.selectAll) this._deselectAll();
	...
	__onActiveItemChanged(e) {
		...
		if (this.autoSelect) {
			...
			if (item) this.__toggleItem(item);
		}
		this.__previousActiveItem = activeItem;
	}
	/** @private */
	__dragAutoScroller() {
		if (this.__dragStartIndex === void 0) return;
		...
		if (this.__dragCurrentY < scrollableArea.top) hoveredIndex = this._grid._firstVisibleIndex;
		else if (this.__dragCurrentY > scrollableArea.bottom) hoveredIndex = this._grid._lastVisibleIndex;
		if (hoveredIndex !== void 0) renderedRows.forEach((row) => {
		...
		const scrollTriggerArea = scrollableArea.height * .15;
	...
	*/
	__toggleItem(item, selected = !this._grid._isSelected(item)) {
		if (selected === this._grid._isSelected(item)) return;
		if (selected) this._selectItem(item);
		else this._deselectItem(item);
	}
	/**
...
//#endregion
export { GridSelectionColumnBaseMixin };
