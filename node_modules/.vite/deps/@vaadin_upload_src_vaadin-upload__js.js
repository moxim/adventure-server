...
import { isTouch } from "./@vaadin_component-base_src_browser-utils__js.js";
...
import { t as getFilesFromDropEvent } from "./vaadin-upload-helpers-gUhRt1Vt.js";
import "./vaadin-upload-file-list-4-087HZ-.js";
//#region node_modules/@vaadin/upload/src/styles/vaadin-upload-icon-base-styles.js
/**
* @license
...
`;
//#endregion
//#region node_modules/@vaadin/upload/src/vaadin-upload-icon.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
//#endregion
//#region node_modules/@vaadin/upload/src/styles/vaadin-upload-base-styles.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
`;
//#endregion
//#region node_modules/@vaadin/upload/src/vaadin-upload-mixin.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var UploadMixin = (superClass) => class UploadMixin extends I18nMixin(superClass) {
	static get properties() {
		return {
			...
			*/
			capture: { type: String },
			/** @private */
			_addButton: { type: Object },
			/** @private */
			_dropLabel: { type: Object },
			/** @private */
			_fileList: { type: Object },
			/** @private */
			_files: { type: Array },
			/** @private */
			_uploadQueue: {
	...
	*/
	...
	get __acceptRegexp() {
		...
		const processedTokens = this.accept.split(",").map((token) => {
			...
			if (processedToken.startsWith("\\.")) processedToken = `.*${processedToken}$`;
	...
	ready() {
		...
		this.addController(new SlotController(this, "file-list", "vaadin-upload-file-list", { initializer: (list) => {
			this._fileList = list;
		} }));
	...
	/** @private */
	_formatSize(bytes) {
		if (typeof this.__effectiveI18n.formatSize === "function") return this.__effectiveI18n.formatSize(bytes);
		const base = this.__effectiveI18n.units.sizeBase || 1e3;
		const unit = ~~(Math.log(bytes) / Math.log(base));
		const dec = Math.max(0, Math.min(3, unit - 1));
		return `${parseFloat((bytes / base ** unit).toFixed(dec))} ${this.__effectiveI18n.units.size[unit]}`;
	}
	/** @private */
	...
	_formatTime(seconds, split) {
		if (typeof this.__effectiveI18n.formatTime === "function") return this.__effectiveI18n.formatTime(seconds, split);
	...
	__updateAddButton(addButton, maxFiles, effectiveI18n, maxFilesReached, disabled) {
		if (addButton) {
			...
			if (addButton === this._addButtonController.defaultNode) addButton.textContent = this._i18nPlural(maxFiles, effectiveI18n.addFiles);
		}
	}
	/** @private */
	__updateDropLabel(dropLabel, maxFiles, effectiveI18n) {
		if (dropLabel && dropLabel === this._dropLabelController.defaultNode) dropLabel.textContent = this._i18nPlural(maxFiles, effectiveI18n.dropFiles);
	}
	/** @private */
	__updateFileList(list, files, effectiveI18n, disabled) {
		if (list) {
			...
			if (this._theme) list.setAttribute("theme", this._theme);
			else list.removeAttribute("theme");
		}
	}
	/** @private */
	_onDragover(event) {
	...
	_onDragleave(event) {
		event.preventDefault();
		if (this._dragover && !this.nodrop) this._dragover = this._dragoverValid = false;
	}
	/** @private */
	async _onDrop(event) {
	...
	_configureXhr(xhr, file = null, isRawUpload = false) {
		if (typeof this.headers === "string") try {
	...
	*/
	_processUploadQueue() {
		while (this._uploadQueue.length > 0 && this._activeUploads < this.maxConcurrentUploads) {
			...
			if (nextFile) this._uploadFile(nextFile);
		}
	}
	/** @private */
	_uploadFile(file) {
		...
		xhr.upload.onprogress = (e) => {
			...
			if (file.error) file.indeterminate = file.status = void 0;
			else if (!file.abort) if (progress < 100) {
			...
			this._renderFileList();
			this.dispatchEvent(new CustomEvent("upload-progress", { detail: {
				file,
				xhr
			} }));
		};
		...
		xhr.onreadystatechange = () => {
			if (xhr.readyState === 4) {
				...
				file.status = "";
				if (!this.dispatchEvent(new CustomEvent("upload-response", {
					detail: {
						file,
						xhr
					},
				...
				if (xhr.status === 0) file.error = this.__effectiveI18n.uploading.error.serverUnavailable;
				else if (xhr.status >= 500) file.error = this.__effectiveI18n.uploading.error.unexpectedServerError;
				else if (xhr.status === 413) file.error = this.__effectiveI18n.uploading.error.fileTooLarge;
				else if (xhr.status >= 400) file.error = this.__effectiveI18n.uploading.error.forbidden;
				file.complete = !file.error;
				this.dispatchEvent(new CustomEvent(`upload-${file.error ? "error" : "success"}`, { detail: {
					file,
					xhr
				} }));
				this._renderFileList();
		...
		if (!file.uploadTarget) file.uploadTarget = this.target || "";
		if (!isRawUpload) file.formDataName = this.formDataName;
		if (!this.dispatchEvent(new CustomEvent("upload-before", {
			detail: {
				file,
				xhr
			},
		...
		let requestBody;
		if (isRawUpload) requestBody = file;
		else {
		...
		xhr.upload.onloadstart = () => {
			this.dispatchEvent(new CustomEvent("upload-start", { detail: {
				file,
				xhr
			} }));
			this._renderFileList();
		...
		if (!isRawUpload) eventDetail.formData = requestBody;
		if (this.dispatchEvent(new CustomEvent("upload-request", {
			detail: eventDetail,
			cancelable: true
	...
	_retryFileUpload(file) {
		if (this.dispatchEvent(new CustomEvent("upload-retry", {
			detail: {
				file,
				xhr: file.xhr
			},
	...
	_abortFileUpload(file) {
		if (this.dispatchEvent(new CustomEvent("upload-abort", {
			detail: {
				file,
				xhr: file.xhr
			},
	...
	/** @private */
	_renderFileList() {
		if (this._fileList && typeof this._fileList.requestContentUpdate === "function") this._fileList.requestContentUpdate();
	}
	/** @private */
	_addFiles(files) {
	...
	*/
	_addFile(file) {
		if (this.maxFilesReached) {
			this.dispatchEvent(new CustomEvent("file-reject", { detail: {
				file,
				error: this.__effectiveI18n.error.tooManyFiles
			} }));
			return;
		}
		if (this.maxFileSize >= 0 && file.size > this.maxFileSize) {
			this.dispatchEvent(new CustomEvent("file-reject", { detail: {
				file,
				error: this.__effectiveI18n.error.fileIsTooBig
			} }));
			return;
		}
		...
		if (re && !(re.test(file.type) || re.test(file.name))) {
			this.dispatchEvent(new CustomEvent("file-reject", { detail: {
				file,
				error: this.__effectiveI18n.error.incorrectFileType
			} }));
			return;
		}
		file.loaded = 0;
		...
		if (!this.noAuto) this._queueFileUpload(file);
	}
	/** @private */
	_updateFocus(fileIndex) {
		...
		if (fileIndex === this.files.length) fileIndex -= 1;
	...
	*/
	...
	/** @private */
	_onAddFilesClick(e) {
		if (this.maxFilesReached) return;
		e.stopPropagation();
	...
	_dragoverChanged(dragover) {
		if (dragover) this.setAttribute("dragover", dragover);
		else this.removeAttribute("dragover");
	}
	/** @private */
	_dragoverValidChanged(dragoverValid) {
		if (dragoverValid) this.setAttribute("dragover-valid", dragoverValid);
		else this.removeAttribute("dragover-valid");
	}
	/** @private */
...
//#endregion
//#region node_modules/@vaadin/upload/src/vaadin-upload.js
/**
* @license
* Copyright (c) 2016 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
