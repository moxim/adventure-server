...
import { a as isElementHidden, i as isElementFocused } from "./focus-utils-CzjQXswF.js";
//#region node_modules/@vaadin/a11y-base/src/keyboard-direction-mixin.js
/**
* @license
...
var KeyboardDirectionMixin = (superclass) => class KeyboardDirectionMixinClass extends KeyboardMixin(superclass) {
	...
	*/
	focus(options) {
		...
		if (idx >= 0) this._focus(idx, options);
	}
	/**
	...
	*/
	_onKeyDown(event) {
		...
		let idx;
		let increment;
		const dirIncrement = !this._vertical && this.getAttribute("dir") === "rtl" ? -1 : 1;
		...
		if (this._tabNavigation && key === "Tab" && (idx > currentIdx && event.shiftKey || idx < currentIdx && !event.shiftKey || idx === currentIdx)) return;
		if (idx >= 0) {
	...
	*/
	__isPrevKeyPressed(key, shiftKey) {
		if (this._vertical) return key === "ArrowUp";
	...
	*/
	__isNextKeyPressed(key, shiftKey) {
		if (this._vertical) return key === "ArrowDown";
	...
	*/
	_getAvailableIndex(items, index, increment, condition) {
		...
		for (let i = 0; typeof idx === "number" && i < totalItems; i += 1, idx += increment || 1) {
			...
			if (this._isItemFocusable(item) && this.__isMatchingItem(item, condition)) return idx;
		}
		return -1;
	}
	/**
...
//#endregion
export { KeyboardDirectionMixin as t };

//# sourceMappingURL=keyboard-direction-mixin-ClXAObhU.js.map