//#region node_modules/@vaadin/upload/src/vaadin-upload-manager.js
/**
* @license
...
*/
var UploadManager = class extends EventTarget {
	...
	*/
	...
	set method(value) {
		if (value !== "POST" && value !== "PUT") throw new Error(`Invalid method "${value}". Only POST and PUT are allowed.`);
		this.#method = value;
	}
	/**
	...
	*/
	...
	set maxFiles(value) {
		if (value < 0) throw new Error(`Invalid maxFiles "${value}". Value must be non-negative.`);
		this.#maxFiles = value;
	...
	*/
	...
	set maxConcurrentUploads(value) {
		if (value <= 0) throw new Error(`Invalid maxConcurrentUploads "${value}". Value must be positive.`);
	...
	*/
	...
	set disabled(value) {
		...
		if (disabled !== this.#disabled) {
			...
			this.dispatchEvent(new CustomEvent("disabled-changed", { detail: { value: disabled } }));
		}
	}
	/**
	...
	*/
	...
	get #acceptRegexp() {
		...
		const processedTokens = this.accept.split(",").map((token) => {
			...
			if (processedToken.startsWith(String.raw`\.`)) processedToken = `.*${processedToken}$`;
	...
	#updateMaxFilesReached() {
		...
		if (reached !== this.#maxFilesReached) {
			...
			this.dispatchEvent(new CustomEvent("max-files-reached-changed", { detail: { value: reached } }));
		}
	}
	/**
	...
	*/
	#validateFile(file, currentCount) {
		if (currentCount >= this.maxFiles) return "tooManyFiles";
		if (this.maxFileSize >= 0 && file.size > this.maxFileSize) return "fileIsTooBig";
		const re = this.#acceptRegexp;
		if (re && !(re.test(file.type) || re.test(file.name))) return "incorrectFileType";
		return null;
	}
	#addFile(file) {
		...
		if (!this.noAuto) this.#queueFileUpload(file);
	}
	#removeFile(file) {
		...
		if (fileIndex >= 0) {
			...
			this.dispatchEvent(new CustomEvent("file-remove", { detail: {
				file,
				fileIndex
			} }));
		}
	...
	#queueFileUpload(file) {
		...
		if (this.#uploadQueue.includes(file)) return;
		file.loaded = 0;
		file.progress = 0;
		file.held = true;
	...
	#processUploadQueue() {
		while (this.#uploadQueue.length > 0 && this.#activeUploads < this.maxConcurrentUploads) {
			...
			if (nextFile) this.#uploadFile(nextFile);
		}
	}
	#uploadFile(file) {
		...
		xhr.upload.onprogress = (e) => {
			...
			if (file.stalled) file.stalled = false;
			if (file.errorKey) file.indeterminate = file.status = void 0;
			else if (!file.abort) {
				if (progress < 100) {
		...
		xhr.ontimeout = () => {
			...
			this.#processUploadQueue();
			this.#cleanupXhr(xhr);
			this.dispatchEvent(new CustomEvent("upload-error", { detail: {
				file,
				xhr
			} }));
			this.#notifyFilesChanged();
		...
		xhr.onreadystatechange = () => {
			if (xhr.readyState === 4) {
				...
				this.#processUploadQueue();
				this.#cleanupXhr(xhr);
				if (file.abort || file.errorKey) return;
				file.status = "";
				...
				if (xhr.status === 0) file.errorKey = "serverUnavailable";
				else if (xhr.status >= 500) file.errorKey = "unexpectedServerError";
				else if (xhr.status === 413) file.errorKey = "fileTooLarge";
				else if (xhr.status >= 400) file.errorKey = "forbidden";
		...
		if (!this.#files.includes(file)) {
			if (!file.abort) this.#activeUploads -= 1;
			this.#cleanupXhr(xhr);
			this.#processUploadQueue();
			return;
		}
...
//# sourceMappingURL=vaadin-upload-manager-C3vVVQLn.js.map