...
//#region node_modules/@vaadin/card/src/styles/vaadin-card-base-styles.js
/**
* @license
...
`;
//#endregion
//#region node_modules/@vaadin/card/src/vaadin-card.js
/**
* @license
* Copyright (c) 2024 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
* @extends HTMLElement
*/
var Card = class extends ElementMixin(ThemableMixin(PolylitMixin(LumoInjectionMixin(LitElement)))) {
	static get is() {
		return "vaadin-card";
	}
	static get styles() {
	...
	_onSlotChange() {
		...
		this.toggleAttribute("_t", this.querySelector(":scope > [slot=\"title\"]") && !this.querySelector(":scope > [slot=\"header\"]"));
		this.toggleAttribute("_st", this.querySelector(":scope > [slot=\"subtitle\"]") && !this.querySelector(":scope > [slot=\"header\"]"));
		...
		if (this.__getCustomTitleElement()) this.__clearStringTitle();
	}
	/** @private */
	__hasContent() {
		return this.shadowRoot.querySelector("slot:not([name])").assignedNodes({ flatten: true }).filter((node) => !isEmptyTextNode(node)).length > 0;
	}
	/** @private */
	__clearStringTitle() {
		const stringTitleElement = this.__getStringTitleElement();
		if (stringTitleElement) this.removeChild(stringTitleElement);
		if (this.getAttribute("aria-labelledby")?.startsWith("card-title-")) this.removeAttribute("aria-labelledby");
		if (this.cardTitle) this.cardTitle = "";
	}
	/** @private */
	...
	__cardTitleChanged(title) {
		...
		if (customTitleElement) this.removeChild(customTitleElement);
	...
	__titleHeadingLevelChanged(titleHeadingLevel) {
		const stringTitleElement = this.__getStringTitleElement();
		if (stringTitleElement) this.__setTitleHeadingLevel(stringTitleElement, titleHeadingLevel);
	}
	/** @private */
...
//# sourceMappingURL=@vaadin_card_src_vaadin-card__js.js.map