//#region node_modules/@vaadin/component-base/src/slot-observer.js
/**
* @license
	...
	*/
	_collectMovedNodes(currentNodes) {
		...
		currentPerSlot.forEach((nodes, slot) => {
			...
			if (new Set(stored).difference(new Set(nodes)).size > 0) return;
			stored.forEach((node, storedIndex) => {
				if (nodes.indexOf(node) !== storedIndex) movedNodes.push(node);
			});
		});
		return movedNodes;
	}
	...
	_processNodes() {
		...
		if (addedNodes.length || removedNodes.length || movedNodes.length || this.forceInitial) this.callback({
			addedNodes,
			currentNodes,
			movedNodes,
			removedNodes
		});
		if (this.forceInitial) this.forceInitial = false;
		this._storedNodes = currentNodes;
	}
};
//#endregion
export { SlotObserver as t };

//# sourceMappingURL=slot-observer-BXFrQCzl.js.map