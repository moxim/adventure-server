//#region node_modules/@vaadin/component-base/src/overflow-controller.js
/**
* @license
...
var OverflowController = class {
	...
	*/
		...
		this.__childObserver = new MutationObserver((mutations) => {
			mutations.forEach(({ addedNodes, removedNodes }) => {
				addedNodes.forEach((node) => {
					if (node.nodeType === Node.ELEMENT_NODE) this.__resizeObserver.observe(node);
				});
				removedNodes.forEach((node) => {
					if (node.nodeType === Node.ELEMENT_NODE) this.__resizeObserver.unobserve(node);
				});
				if (addedNodes.length === 0 && removedNodes.length > 0) this.__updateState({ sync: true });
			});
		});
	...
	__updateState({ sync }) {
		...
		if (sync) this.__writeState(state);
		else this.__resizeRaf = requestAnimationFrame(() => this.__writeState(state));
	}
	/** @private */
	__readState() {
		const target = this.scrollTarget;
		let overflow = "";
		if (target.scrollTop > 0) overflow += " top";
		if (Math.ceil(target.scrollTop) < Math.ceil(target.scrollHeight - target.clientHeight)) overflow += " bottom";
		const scrollLeft = Math.abs(target.scrollLeft);
		if (scrollLeft > 0) overflow += " start";
		if (Math.ceil(scrollLeft) < Math.ceil(target.scrollWidth - target.clientWidth)) overflow += " end";
		return { overflow: overflow.trim() };
	}
	/** @private */
	__writeState({ overflow }) {
		if (overflow) this.host.setAttribute("overflow", overflow);
		else this.host.removeAttribute("overflow");
	}
};
//#endregion
export { OverflowController as t };

//# sourceMappingURL=overflow-controller-B1O3h4Je.js.map