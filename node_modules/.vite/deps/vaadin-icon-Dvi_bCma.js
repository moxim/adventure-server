...
import { a as unsafeSvgLiteral, r as ensureSvgLiteral } from "./vaadin-iconset-D7dLhf66.js";
//#region node_modules/@vaadin/icon/src/styles/vaadin-icon-base-styles.js
/**
* @license
...
`;
//#endregion
//#region node_modules/@vaadin/icon/src/vaadin-icon-helpers.js
/**
* @license
...
function supportsCQUnitsForPseudoElements() {
	...
	const needsFallback = [...document.querySelectorAll(".vaadin-icon-test-element")].find((el) => getComputedStyle(el, "::before").height !== "2px");
	testStyle.remove();
	testElement.remove();
	shadowParent.remove();
...
function needsFontIconSizingFallback() {
	if (!CSS.supports("container-type: inline-size")) return true;
	if (!isSafari) return false;
	return !supportsCQUnitsForPseudoElements();
}
//#endregion
//#region node_modules/@vaadin/icon/src/vaadin-icon-font-size-mixin.js
/**
* @license
* Copyright (c) 2021 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var IconFontSizeMixin = (superclass) => !usesFontIconSizingFallback ? superclass : class extends ResizeMixin(superclass) {
	...
	updated(props) {
		super.updated(props);
		if (props.has("char") || props.has("iconClass") || props.has("ligature")) this.__updateFontIconSize();
	}
	/**
	* @protected
	* @override
...
//#endregion
//#region node_modules/@vaadin/icon/src/vaadin-icon-mixin.js
/**
* @license
* Copyright (c) 2021 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var IconMixin = (superClass) => class extends IconFontSizeMixin(superClass) {
	...
	_applyIcon() {
		...
		if (viewBox) this.__viewBox = viewBox;
		if (preserveAspectRatio) this.__preserveAspectRatio = preserveAspectRatio;
		if (size && size !== this.size) this.size = size;
		this.svg = svg;
	}
	/** @private */
	__iconChanged(icon) {
		if (icon) this._applyIcon();
		else this.svg = null;
	}
	/** @private */
	async __srcChanged(src, symbol) {
		...
		if (!src.startsWith("data:") && (symbol || src.includes("#"))) {
			...
			if (!srcCache.has(src)) srcCache.set(src, this.__fetch(src, { mode: "cors" }).then((data) => {
				if (!data.ok) throw new Error("Error loading icon");
				return data.text();
			}));
			const svgData = await srcCache.get(src);
			if (!superClass.__domParser) superClass.__domParser = new DOMParser();
			const svgElement = superClass.__domParser.parseFromString(svgData, "text/html").querySelector("svg");
			if (!svgElement) throw new Error(`SVG element not found on path: ${src}`);
			this.svg = unsafeSvgLiteral(svgElement.innerHTML);
			if (symbol) this.__useRef = `#${symbol}`;
	...
	__fontChanged(iconClass, char, ligature) {
		...
		if (char) this.setAttribute("font-icon-content", char.length > 1 ? String.fromCodePoint(parseInt(char, 16)) : char);
		else if (ligature) this.setAttribute("font-icon-content", ligature);
		else this.removeAttribute("font-icon-content");
		if ((iconClass || char || ligature) && !this.icon) this.icon = "";
	}
	/** @protected */
	attributeChangedCallback(name, oldValue, newValue) {
		super.attributeChangedCallback(name, oldValue, newValue);
		if (name === "class" && this.__iconClasses.some((className) => !this.classList.contains(className))) this.classList.add(...this.__iconClasses);
	}
	/** @private */
...
//#endregion
//#region node_modules/@vaadin/icon/src/vaadin-icon.js
/**
* @license
* Copyright (c) 2021 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
//# sourceMappingURL=vaadin-icon-Dvi_bCma.js.map