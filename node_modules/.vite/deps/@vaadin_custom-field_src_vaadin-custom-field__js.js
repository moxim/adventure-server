...
import { a as getFlattenedElements } from "./dom-utils-DDAqRijW.js";
...
//#region node_modules/@vaadin/custom-field/src/vaadin-custom-field-mixin.js
/**
* @license
...
var CustomFieldMixin = (superClass) => class CustomFieldMixin extends FieldMixin(FocusMixin(KeyboardMixin(superClass))) {
	...
	ready() {
		...
		this._tooltipController.setShouldShow((target) => {
			return !(target.inputs || []).some((el) => el.opened);
		});
	}
	/**
	...
	*/
	focus(options) {
		if (this.inputs && this.inputs[0]) this.inputs[0].focus(options);
	}
	/**
	...
	*/
	_setFocused(focused) {
		...
		if (!focused) this._requestValidation();
	}
	/**
	...
	*/
	checkValidity() {
		if (this.inputs && this.inputs.some((input) => !(input.validate || input.checkValidity).call(input)) || this.required && !(this.value && this.value.trim())) return false;
		return true;
	}
	/**
	...
	*/
	_onKeyDown(e) {
		if (e.key === "Tab") {
			...
			if (inputs.indexOf(e.target) < inputs.length - 1 && !e.shiftKey || inputs.indexOf(e.target) > 0 && e.shiftKey) return;
			this.__setValue();
		}
	}
	/** @protected */
	...
	/** @private */
	__isInput(node) {
		return !(node.getAttribute("slot") === "input" || node.getAttribute("slot") === "textarea") && (node.validate || node.checkValidity);
	}
	/** @private */
	__getInputsFromSlot() {
	...
	__inputsChanged(inputs, oldInputs) {
		if (inputs.length === 0) {
			if (oldInputs?.length > 0) this.__setValue();
			return;
		}
		if (this.value && this.value !== "	" && (!oldInputs || oldInputs.length === 0)) this.__applyInputsValue(this.value);
		else this.__setValue();
	}
	/** @private */
	...
	__valueChanged(value, oldValue) {
		...
		if (this.__settingValue || !this.inputs) return;
		this.__applyInputsValue(value || "	");
		if (oldValue !== void 0) this._requestValidation();
	}
	/** @private */
	__applyInputsValue(value) {
		const valuesArray = (this.parseValue || defaultParseValue).apply(this, [value]);
...
//#endregion
//#region node_modules/@vaadin/custom-field/src/vaadin-custom-field.js
/**
* @license
* Copyright (c) 2019 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
