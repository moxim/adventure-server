...
import { a as isElementHidden, o as isKeyboardActive, t as getDeepActiveElement } from "./focus-utils-CzjQXswF.js";
...
//#region node_modules/@vaadin/overlay/src/styles/vaadin-overlay-base-styles.js
/**
* @license
...
`;
//#endregion
//#region node_modules/@vaadin/a11y-base/src/focus-restoration-controller.js
/**
* @license
...
var FocusRestorationController = class {
	...
	*/
	restoreFocus(options) {
		...
		if (getDeepActiveElement() === document.body) setTimeout(() => focusNode.focus(focusOptions));
		else focusNode.focus(focusOptions);
		this.focusNode = null;
	}
};
//#endregion
//#region node_modules/@vaadin/overlay/src/vaadin-overlay-focus-mixin.js
/**
* @license
* Copyright (c) 2017 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var OverlayFocusMixin = (superClass) => class OverlayFocusMixin extends superClass {
	...
	*/
	_saveFocus() {
		if (this.restoreFocusOnClose) this.__focusRestorationController.saveFocus(this.restoreFocusNode);
	}
	/**
	...
	*/
	_trapFocus() {
		if (this.focusTrap && !isElementHidden(this._focusTrapRoot)) this.__focusTrapController.trapFocus(this._focusTrapRoot);
	}
	/**
	...
	*/
	_deepContains(node) {
		if (this._contentRoot.contains(node)) return true;
		let n = node;
		const doc = node.ownerDocument;
		while (n && n !== doc && n !== this._contentRoot) n = n.parentNode || n.host;
		return n === this._contentRoot;
	}
};
//#endregion
//#region node_modules/@vaadin/overlay/src/vaadin-overlay-stack-mixin.js
/**
* @license
* Copyright (c) 2017 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var hasOnlyNestedOverlays = (overlay) => {
	...
	if (!next) return true;
	if (!overlay._deepContains(next)) return false;
...
var isLastOverlay = (overlay, filter = (_overlay) => true) => {
	return overlay === getAttachedInstances().filter(filter).pop();
};
var OverlayStackMixin = (superClass) => class OverlayStackMixin extends superClass {
	...
	*/
	bringToFront() {
		if (isLastOverlay(this) || hasOnlyNestedOverlays(this)) return;
	...
	_enterModalState() {
		...
		getAttachedInstances().forEach((el) => {
			if (el !== this) el.$.overlay.style.pointerEvents = "none";
		});
	}
	/** @protected */
	_exitModalState() {
	...
	_removeAttachedInstance() {
		if (this._isAttached) attachedInstances.delete(this);
	}
};
//#endregion
//#region node_modules/@vaadin/overlay/src/vaadin-overlay-utils.js
/**
* @license
...
function setOverlayStateAttribute(overlay, name, value) {
	...
	if (overlay.owner) elements.push(overlay.owner);
	if (typeof value === "string") elements.forEach((element) => {
		element.setAttribute(name, value);
	});
	else if (value) elements.forEach((element) => {
...
//#endregion
//#region node_modules/@vaadin/overlay/src/vaadin-overlay-mixin.js
/**
* @license
* Copyright (c) 2017 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var OverlayMixin = (superClass) => class OverlayMixin extends OverlayFocusMixin(OverlayStackMixin(superClass)) {
	...
	*/
	...
	constructor() {
		...
		/* c8 ignore next 3 */
		if (isIOS) this._boundIosResizeListener = () => this._detectIosNavbar();
	}
	/** @protected */
	firstUpdated() {
		super.firstUpdated();
		...
		this.addEventListener("mouseup", () => {
			if (document.activeElement === document.body && this.$.overlay.getAttribute("tabindex") === "0") this.$.overlay.focus();
		});
	...
	connectedCallback() {
		...
		if (this.opened) this._attachOverlay();
	}
	/** @protected */
	disconnectedCallback() {
		...
		/* c8 ignore next 3 */
		if (this._boundIosResizeListener) window.removeEventListener("resize", this._boundIosResizeListener);
	}
	/**
	...
	*/
	requestContentUpdate() {
		if (this.renderer) this.renderer.call(this.owner, this._rendererRoot, this.owner, this.model);
	}
	/**
	...
	*/
	close(sourceEvent) {
		...
		if (!event.defaultPrevented) this.opened = false;
	}
	/**
	...
	*/
	setBounds(bounds, absolute = true) {
		...
		if (absolute && overlay.style.position !== "absolute") overlay.style.position = "absolute";
		Object.keys(parsedBounds).forEach((arg) => {
			if (parsedBounds[arg] !== null && !isNaN(parsedBounds[arg])) parsedBounds[arg] = `${parsedBounds[arg]}px`;
		});
	...
	_detectIosNavbar() {
		...
		else this.style.setProperty("--vaadin-overlay-viewport-bottom", "0px");
	}
	/**
	...
	*/
	...
	/** @private */
	_addGlobalListeners() {
		if (this.__hasGlobalListeners) return;
	...
	_rendererOrDataChanged(renderer, owner, model, opened) {
		...
		if (opened && renderer && (rendererChanged || openedChanged || ownerOrModelChanged)) this.requestContentUpdate();
	}
	/** @private */
	_modelessChanged(modeless) {
		if (this.opened) if (this._shouldAddGlobalListeners()) this._addGlobalListeners();
		else this._removeGlobalListeners();
		if (!modeless) {
			if (this.opened) this._enterModalState();
		} else this._exitModalState();
	...
	_openedChanged(opened, wasOpened) {
		if (opened) {
			...
			if (this._shouldAddGlobalListeners()) this._addGlobalListeners();
		} else if (wasOpened) {
			if (this.__scheduledOpen) {
				cancelAnimationFrame(this.__scheduledOpen);
				this.__scheduledOpen = null;
			...
			if (this._shouldAddGlobalListeners()) this._removeGlobalListeners();
		}
	}
	/** @private */
	...
	*/
	_shouldAnimate() {
		...
		return !(style.getPropertyValue("display") === "none") && name && name !== "none";
	}
	/**
	* @param {string} type
	* @param {Function} callback
	...
	*/
	...
	/** @private */
	_animatedOpening() {
		if (this._isAttached && this.hasAttribute("closing")) this._flushAnimation("closing");
		this._attachOverlay();
		this._appendAttachedInstance();
		this.bringToFront();
	...
	*/
	_outsideClickListener(event) {
		...
		if (!this._shouldCloseOnOutsideClick(event)) return;
	...
	*/
	_keydownListener(event) {
		if (!this._last || event.defaultPrevented) return;
		if (!this._shouldAddGlobalListeners() && !event.composedPath().includes(this._focusTrapRoot)) return;
		if (event.key === "Escape") {
...
//#endregion
export { isLastOverlay as a, overlayStyles as c, hasOnlyNestedOverlays as i, observeMove as n, OverlayFocusMixin as o, setOverlayStateAttribute as r, FocusRestorationController as s, OverlayMixin as t };

//# sourceMappingURL=vaadin-overlay-mixin-Cp6vfu5u.js.map