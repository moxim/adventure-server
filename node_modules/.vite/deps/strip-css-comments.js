//#region node_modules/is-regexp/index.js
var { toString } = Object.prototype;
function isRegexp(value) {
	return toString.call(value) === "[object RegExp]";
}
//#endregion
//#region node_modules/strip-css-comments/index.js
function stripCssComments(cssString, { preserve = true, whitespace = true, all } = {}) {
	if (all) throw new Error("The `all` option is no longer supported. Use the `preserve` option instead.");
	...
	for (let index = 0; index < cssString.length; index++) {
		...
		if (cssString[index - 1] !== "\\" && (currentCharacter === "\"" || currentCharacter === "'")) {
			if (isInsideString === currentCharacter) isInsideString = false;
			else if (!isInsideString) isInsideString = currentCharacter;
		}
		if (!isInsideString && currentCharacter === "/" && cssString[index + 1] === "*") {
			...
			for (; index2 < cssString.length; index2++) {
				if (cssString[index2] === "*" && cssString[index2 + 1] === "/") {
					if (preserveImportant && isImportantComment || preserveFilter && preserveFilter(comment)) returnValue += `/*${comment}*/`;
					else if (!whitespace) {
...
//#endregion
export { stripCssComments as default };
