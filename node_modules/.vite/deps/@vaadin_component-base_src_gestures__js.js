...
//#region node_modules/@vaadin/component-base/src/gestures.js
/**
@license
...
function isSyntheticClick(ev) {
	if (ev.type === "click") {
		...
		if (!t.nodeType || t.nodeType !== Node.ELEMENT_NODE) return true;
...
function _handleNative(ev) {
	const type = ev.type;
	const gobj = ev.currentTarget[GESTURE_KEY];
	if (!gobj) return;
	const gs = gobj[type];
	if (!gs) return;
	if (!ev[HANDLED_OBJ]) {
...
function _handleTouchAction(ev) {
	...
	if (type === "touchstart") {
		...
		if (!ev.cancelable) {} else if (ta === "none") shouldPrevent = true;
...
function setTouchAction(node, value) {
	if (HAS_NATIVE_TA && node instanceof HTMLElement) microTask.run(() => {
...
function _fire(target, type, detail) {
	...
	if (ev.defaultPrevented) {
		...
		if (preventer?.preventDefault) preventer.preventDefault();
	}
}
/**
	...
	mousedown(e) {
		...
		const movefn = (e) => {
			...
			if (trackHasMovedEnough(self.info, x, y)) {
				...
				if (self.info.state === "start") prevent("tap");
				self.info.addMove({
					x,
					y
				});
	...
	*/
	touchmove(e) {
		...
		if (trackHasMovedEnough(this.info, x, y)) {
			if (this.info.state === "start") prevent("tap");
			this.info.addMove({
				x,
				y
			});
...
function trackForward(info, e, preventer) {
	...
	if (!t || canBeDisabled[t.localName] && t.hasAttribute("disabled")) return;
...
//#endregion
export { addListener, deepTargetFind, gestures, prevent, recognizers, register, removeListener, setTouchAction };
