...
import { t as FieldMixin } from "./field-mixin-De6xCUgx.js";
import { t as inputFieldShared } from "./input-field-shared-styles-B8aX35No.js";
import { dateEquals, formatUTCISODate, normalizeUTCDate, parseUTCDate } from "./@vaadin_date-picker_src_vaadin-date-picker-helper__js.js";
import { n as datePickerI18nDefaults } from "./vaadin-date-picker-DWIHqXdI.js";
import { n as timePickerI18nDefaults } from "./vaadin-time-picker-B0vLqB3G.js";
import { formatISOTime, parseISOTime, validateTime } from "./@vaadin_time-picker_src_vaadin-time-picker-helper__js.js";
//#region node_modules/@vaadin/date-time-picker/src/styles/vaadin-date-time-picker-base-styles.js
/**
* @license
* Copyright (c) 2019 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
*/
var dateTimePickerStyles = css`
...
`;
//#endregion
//#region node_modules/@vaadin/date-time-picker/src/vaadin-date-time-picker-mixin.js
/**
* @license
* Copyright (c) 2019 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
var PickerSlotController = class extends SlotController {
	constructor(host, type) {
		super(host, `${type}-picker`, `vaadin-${type}-picker`, { initializer: (picker, host) => {
...
*/
var DateTimePickerMixin = (superClass) => class DateTimePickerMixinClass extends I18nMixin(FieldMixin(FocusMixin(DisabledMixin(superClass)))) {
	...
	*/
	get __unparsableValue() {
		if (this.__filledPickers.length > 0 && !this.__pickers.every((picker) => picker.value)) return this.__pickers.map((picker) => picker.value || picker.__unparsableValue).join("T");
		return "";
	}
	/** @protected */
	ready() {
		...
		if (this.autofocus && !this.disabled) window.requestAnimationFrame(() => this.focus());
	...
	*/
	focus(options) {
		if (this.__datePicker) this.__datePicker.focus(options);
	}
	/** @private */
	__onGlobalClick(event) {
		if (!(this.__datePicker.opened || this.__timePicker.opened)) return;
		if (event.composedPath().every((node) => {
			return ![
				this.__datePicker,
	...
	*/
	_setFocused(focused) {
		...
		if (!focused && document.hasFocus()) this.__commitPendingValueChange();
	}
	/**
	...
	*/
	_shouldRemoveFocus(event) {
		...
		if (this.__datePicker.opened || this.__timePicker.opened || this.__datePicker.contains(target) || this.__timePicker.contains(target)) return false;
		return true;
	}
	/** @private */
	__syncI18n(target, i18n, props) {
		const targetI18n = {};
		props.forEach((prop) => {
			if (i18n?.hasOwnProperty(prop)) targetI18n[prop] = i18n[prop];
		});
	...
	__changeEventHandler(event) {
		...
		if (filledPickers.length === 1 && filledPickers[0].checkValidity() && !isAlreadyInvalid) return;
		if (this.__hasPendingValueChange) this.__commitPendingValueChange();
	}
	/** @private */
	__openedChangedEventHandler() {
		...
		if (!opened && this.__outsideClickInProgress) this.__commitPendingValueChange();
	}
	/** @private */
	...
	__updateTimePickerMinMax() {
		if (this.__timePicker && this.__datePicker) {
			...
			if (this.__minDateTime && dateEquals(selectedDate, this.__minDateTime, normalizeUTCDate) || isMinMaxSameDay) this.__timePicker.min = this.__dateToIsoTimeString(this.__minDateTime);
			else this.__timePicker.min = this.__defaultTimeMinValue;
			if (this.__maxDateTime && dateEquals(selectedDate, this.__maxDateTime, normalizeUTCDate) || isMinMaxSameDay) this.__timePicker.max = this.__dateToIsoTimeString(this.__maxDateTime);
			else this.__timePicker.max = this.__defaultTimeMaxValue;
		}
	}
	/** @private */
	__i18nChanged(effectiveI18n, datePicker, timePicker) {
		if (datePicker && this.__isDefaultPicker(datePicker, "date")) this.__syncI18n(datePicker, effectiveI18n, datePickerI18nProps);
		if (timePicker && this.__isDefaultPicker(timePicker, "time")) this.__syncI18n(timePicker, effectiveI18n, timePickerI18nProps);
	}
	/** @private */
	__labelOrAccessibleNameChanged(label, accessibleName, effectiveI18n, datePicker, timePicker) {
		const name = accessibleName || label || "";
		if (datePicker) datePicker.accessibleName = `${name} ${effectiveI18n.dateLabel || ""}`.trim();
		if (timePicker) timePicker.accessibleName = `${name} ${effectiveI18n.timeLabel || ""}`.trim();
	}
	/** @private */
	__datePlaceholderChanged(datePlaceholder, datePicker) {
		if (datePicker) datePicker.placeholder = datePlaceholder;
	}
	/** @private */
	__timePlaceholderChanged(timePlaceholder, timePicker) {
		if (timePicker) timePicker.placeholder = timePlaceholder;
	}
	/** @private */
	__stepChanged(step, timePicker) {
		if (timePicker && timePicker.step !== step) timePicker.step = step;
	}
	/** @private */
	__initialPositionChanged(initialPosition, datePicker) {
		if (datePicker) datePicker.initialPosition = initialPosition;
	}
	/** @private */
	__showWeekNumbersChanged(showWeekNumbers, datePicker) {
		if (datePicker) datePicker.showWeekNumbers = showWeekNumbers;
	}
	/** @private */
	__invalidChanged(invalid, datePicker, timePicker) {
		if (datePicker) datePicker.invalid = invalid;
		if (timePicker) timePicker.invalid = invalid;
	}
	/** @private */
	__requiredChanged(required, datePicker, timePicker) {
		if (datePicker) datePicker.required = required;
		if (timePicker) timePicker.required = required;
		if (this.__oldRequired && !required) this._requestValidation();
		this.__oldRequired = required;
	}
	/** @private */
	__disabledChanged(disabled, datePicker, timePicker) {
		if (datePicker) datePicker.disabled = disabled;
		if (timePicker) timePicker.disabled = disabled;
	}
	/** @private */
	__readonlyChanged(readonly, datePicker, timePicker) {
		if (datePicker) datePicker.readonly = readonly;
		if (timePicker) timePicker.readonly = readonly;
	}
	/**
	...
	*/
	__formatDateTime(date) {
		if (!date) return "";
		return `${this.__formatDateISO(date, "")}T${this.__dateToIsoTimeString(date)}`;
	}
	/**
	...
	*/
	__dateToIsoTimeString(date) {
		return formatISOTime(validateTime({
	...
	*/
	...
	__commitPendingValueChange() {
		...
		if (this.__committedValue !== this.value) this.dispatchEvent(new CustomEvent("change", { bubbles: true }));
		else if (this.__committedUnparsableValue !== this.__unparsableValue) this.dispatchEvent(new CustomEvent("unparsable-change"));
	...
	*/
	__dateTimeEquals(date1, date2) {
		if (!dateEquals(date1, date2, normalizeUTCDate)) return false;
	...
	__handleDateTimeChange(property, parsedProperty, value, oldValue) {
		...
		if (!this.__dateTimeEquals(this[parsedProperty], dateTime)) this[parsedProperty] = dateTime;
	}
	/** @private */
	...
	__minChanged(value, oldValue) {
		...
		if (this.__datePicker) this.__datePicker.min = this.__formatDateISO(this.__minDateTime, this.__defaultDateMinMaxValue);
		this.__updateTimePickerMinMax();
		if (this.__datePicker && this.__timePicker && this.value) this._requestValidation();
	}
	/** @private */
	__maxChanged(value, oldValue) {
		this.__handleDateTimeChange("max", "__maxDateTime", value, oldValue);
		if (this.__datePicker) this.__datePicker.max = this.__formatDateISO(this.__maxDateTime, this.__defaultDateMinMaxValue);
		this.__updateTimePickerMinMax();
		if (this.__datePicker && this.__timePicker && this.value) this._requestValidation();
	}
	/** @private */
	__selectedDateTimeChanged(selectedDateTime) {
		...
		if (Boolean(this.__datePicker && this.__datePicker.$) && !this.__ignoreInputValueChange) {
	...
	__valueChangedEventHandler() {
		if (this.__ignoreInputValueChange) return;
		this.__ignoreInputValueChange = true;
	...
	__autoOpenDisabledChanged(autoOpenDisabled, datePicker, timePicker) {
		if (datePicker) datePicker.autoOpenDisabled = autoOpenDisabled;
		if (timePicker) timePicker.autoOpenDisabled = autoOpenDisabled;
	}
	/** @private */
	__themeChanged(theme, datePicker, timePicker) {
		if (!datePicker || !timePicker) return;
		[datePicker, timePicker].forEach((picker) => {
			if (theme) picker.setAttribute("theme", theme);
			else picker.removeAttribute("theme");
		});
	}
	/** @private */
	__pickersChanged(datePicker, timePicker) {
		if (!datePicker || !timePicker) return;
		if (this.__isDefaultPicker(datePicker, "date") !== this.__isDefaultPicker(timePicker, "time")) return;
		if (datePicker.value) this.__valueChangedEventHandler();
		else if (this.value) {
			this.__selectedDateTimeChanged(this.__selectedDateTime);
			if (this.min && this.__minDateTime || this.max && this.__maxDateTime) this._requestValidation();
		}
	}
};
//#endregion
//#region node_modules/@vaadin/date-time-picker/src/vaadin-date-time-picker.js
/**
* @license
* Copyright (c) 2019 - 2026 Vaadin Ltd.
* This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
...
* @extends HTMLElement
*/
var DateTimePicker = class extends DateTimePickerMixin(ThemableMixin(ElementMixin(PolylitMixin(LumoInjectionMixin(LitElement))))) {
	static get is() {
...
//# sourceMappingURL=@vaadin_date-time-picker_src_vaadin-date-time-picker__js.js.map